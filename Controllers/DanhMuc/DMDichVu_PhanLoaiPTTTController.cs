...
{
    public class DMDichVu_PhanLoaiPTTTController : BCAController
    {
        #region Khởi tạo
        private IMemoryCache cache;
        //public readonly ISession session;
        public DMDichVu_PhanLoaiPTTTController(IMemoryCache cache)
        {
            this.cache = cache;
        ...
        [HttpGet]
        public JsonResult LoadData(string MaPLPTTT, string TenPLPTTT, string maMay, string ngaySD, string nguoiSD, int pageIndex, int pageSize, int add)
        {
            //var response = _DichVu_PhanLoaiPTTTService.DMDichVu_PhanLoaiPTTTGetListPaging(MaPLPTTT, TenPLPTTT, maMay, ngaySD, nguoiSD, pageIndex, pageSize, add);
            //return CreateJsonJsonResult(() =>
            //{
            //    return Json(response, JsonRequestBehavior.AllowGet);
            //});
            Pagination db = new Pagination();
            if (string.IsNullOrEmpty(MaPLPTTT))
                MaPLPTTT = "";
            ...
            if (string.IsNullOrEmpty(nguoiSD))
                nguoiSD = "";
            var response = db.DMDichVu_PhanLoaiPTTTGetListPaging(MaPLPTTT, TenPLPTTT, maMay, ngaySD, nguoiSD, pageIndex, pageSize, add);
            return CreateJsonJsonResult(() =>
            {
        ...
        [HttpPost]
        public ActionResult Modify(DMDichVu_PhanLoaiPTTT DichVu_PhanLoaiPTTT)
        {
            var u = SessionHelper.GetObjectFromJson<UserProfileSessionData>(HttpContext.Session, "UserProfileSessionData");
            var Quyen = u.ListRoleSession.Where(x => x.ActionDetailsName == "/DMDichVu_PhanLoaiPTTT/Index/Modify").Count() > 0;
            if (Quyen)
            {
                ...
                emp.TenBang = "DMDichVu_PhanLoaiPTTT";
                emp.KieuTacDong = "Update";
                emp.NguoiSD = u.Pub_sNguoiSD;
                emp.MaMay = MaMay;
                emp.NoiDungSD = JsonConvert.SerializeObject(DichVu_PhanLoaiPTTT);
                collection.InsertOne(emp);
                #endregion
                db.spDMDichVu_PhanLoaiPTTT_UPDATE(DichVu_PhanLoaiPTTT.MaPLPTTT, DichVu_PhanLoaiPTTT.TenPLPTTT, MaMay, u.Pub_sNguoiSD, DichVu_PhanLoaiPTTT.Huy);
        ...
        [HttpPost]
        public ActionResult DeleteDichVu_PhanLoaiPTTT(string MaPLPTTT)
        {
            var u = SessionHelper.GetObjectFromJson<UserProfileSessionData>(HttpContext.Session, "UserProfileSessionData");
            var Quyen = u.ListRoleSession.Where(x => x.ActionDetailsName == "/DMDichVu_PhanLoaiPTTT/Index/Delete").Count() > 0;
            if (Quyen)
            {
                HL7CoreDataDataContext db = new HL7CoreDataDataContext();
                string MaMay = this.GetLocalIPAddress();
                ...
                emp.NguoiSD = u.Pub_sNguoiSD;
                emp.MaMay = MaMay;
                emp.NoiDungSD = "Delete MaPLPTTT: " + MaPLPTTT;
                collection.InsertOne(emp);
                #endregion
                db.spDMDichVu_PhanLoaiPTTT_DELETED(MaPLPTTT, MaMay, u.Pub_sNguoiSD);
        ...
        [HttpPost]
        public ActionResult CreateDichVu_PhanLoaiPTTT(string TenPLPTTT)
        {
            var u = SessionHelper.GetObjectFromJson<UserProfileSessionData>(HttpContext.Session, "UserProfileSessionData");
            var Quyen = u.ListRoleSession.Where(x => x.ActionDetailsName == "/DMDichVu_PhanLoaiPTTT/Index/Create").Count() > 0;
            if (Quyen)
            {
                string MaMay = this.GetLocalIPAddress();
                HL7CoreDataDataContext db = new HL7CoreDataDataContext();
                ...
                emp.NguoiSD = u.Pub_sNguoiSD;
                emp.MaMay = MaMay;
                emp.NoiDungSD = "Create TenPLPTTT: " + TenPLPTTT;
                collection.InsertOne(emp);
                #endregion
                db.spDMDichVu_PhanLoaiPTTT_CREATE(TenPLPTTT, MaMay, u.Pub_sNguoiSD);
        ...
        //public JsonResult DMDichVu_PhanLoaiPTTTGetAll()
        //{
        //    var response = _DichVu_PhanLoaiPTTTService.DMDichVu_PhanLoaiPTTTGetAll();
        //    return CreateJsonJsonResult(() =>
        //    {
        //        return Json(response, JsonRequestBehavior.AllowGet);
        //    });
        ...
        public ActionResult ExportExcel(string obj)
        {
            ...
            HL7CoreDataDataContext db = new HL7CoreDataDataContext();
            var dataSource = db.ExportDMDichVu_PhanLoaiPTTT(key, columns);
            string filename = "DMDichVu_PhanLoaiPTTT " + DateTime.Now.ToString() + ".xlsx";
            //MVC4 mVC4 = new MVC4();
            //mVC4.
        ...
        {
            var u = SessionHelper.GetObjectFromJson<UserProfileSessionData>(HttpContext.Session, "UserProfileSessionData");
            var Quyen = u.ListRoleSession.Where(x => x.ActionDetailsName == "/DMDichVu_PhanLoaiPTTT/Index/ImportDanhMuc").Count() > 0;
            if (Quyen)
            {
                ...
                if (file != null && file.Length > 0)
                {
                    ...
                    reader.Close();
                    result.Tables[0].TableName = "DMDichVu_PhanLoaiPTTT";
                    ...
                    collection.InsertOne(emp);
                    #endregion
                    db.ImportDMDichVu_PhanLoaiPTTT(result.Tables[0], insert);
