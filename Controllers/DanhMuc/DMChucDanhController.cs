...
{
    public class DMChucDanhController : BCAController
    {
        #region Khởi tạo
        private IMemoryCache cache;
        //public readonly ISession session;
        public DMChucDanhController(IMemoryCache cache)
        {
            this.cache = cache;
        ...
        [HttpGet]
        public JsonResult LoadData(string maCD, string tenCD, string maMay, string ngaySD, string nguoiSD, int pageIndex, int pageSize, int add)
        {
            ...
            if (string.IsNullOrEmpty(nguoiSD))
                nguoiSD = "";
            var response = db.DMChucDanhGetListPaging(maCD, tenCD, maMay, ngaySD, nguoiSD, pageIndex, pageSize, add);
            return CreateJsonJsonResult(() =>
            {
        ...
        [HttpPost]
        public ActionResult Modify(DMChucDanh chucDanh)
        {
            var u = SessionHelper.GetObjectFromJson<UserProfileSessionData>(HttpContext.Session, "UserProfileSessionData");
            var Quyen = u.ListRoleSession.Where(x => x.ActionDetailsName == "/DMChucDanh/Index/Modify").Count() > 0;
            if (Quyen)
            {
                ...
                emp.TenBang = "DMChucDanh";
                emp.KieuTacDong = "Update";
                emp.NguoiSD = u.Pub_sNguoiSD;
                emp.MaMay = MaMay;
                emp.NoiDungSD = JsonConvert.SerializeObject(chucDanh);
                collection.InsertOne(emp);
                #endregion
                db.spDMCHUCDanh_UPDATE(chucDanh.MaCD, chucDanh.TenCD, MaMay, u.Pub_sNguoiSD, chucDanh.Huy);
        ...
        [HttpPost]
        public ActionResult DeleteChucDanh(string maCD)
        {
            var u = SessionHelper.GetObjectFromJson<UserProfileSessionData>(HttpContext.Session, "UserProfileSessionData");
            var Quyen = u.ListRoleSession.Where(x => x.ActionDetailsName == "/DMChucDanh/Index/Delete").Count() > 0;
            if (Quyen)
            {
                HL7CoreDataDataContext db = new HL7CoreDataDataContext();
                string MaMay = this.GetLocalIPAddress();
                ...
                emp.NguoiSD = u.Pub_sNguoiSD;
                emp.MaMay = MaMay;
                emp.NoiDungSD = "Delete maCD: " + maCD;
                collection.InsertOne(emp);
                #endregion
                db.spDMCHUCDanh_DELETED(maCD, MaMay, u.Pub_sNguoiSD);
        ...
        [HttpPost]
        public ActionResult CreateChucDanh(string tenCD)
        {
            var u = SessionHelper.GetObjectFromJson<UserProfileSessionData>(HttpContext.Session, "UserProfileSessionData");
            var Quyen = u.ListRoleSession.Where(x => x.ActionDetailsName == "/DMChucDanh/Index/Create").Count() > 0;
            if (Quyen)
            {
                string MaMay = this.GetLocalIPAddress();
                HL7CoreDataDataContext db = new HL7CoreDataDataContext();
                ...
                emp.NguoiSD = u.Pub_sNguoiSD;
                emp.MaMay = MaMay;
                emp.NoiDungSD = "Create tenCD: " + tenCD;
                collection.InsertOne(emp);
                #endregion
                db.spDMCHUCDanh_CREATE(tenCD, MaMay, u.Pub_sNguoiSD);
        ...
        //public JsonResult DMChucDanhGetAll()
        //{
        //    var response = _chucDanhService.DMChucDanhGetAll();
        //    return CreateJsonJsonResult(() =>
        //    {
        //        return Json(response, JsonRequestBehavior.AllowGet);
        //    });
        ...
        public ActionResult ExportExcel(string obj)
        {
            ...
            HL7CoreDataDataContext db = new HL7CoreDataDataContext();
            var dataSource = db.ExportDMChucDanh(key, columns);
            string filename = "DMChucDanh " + DateTime.Now.ToString() + ".xlsx";
            //MVC4 mVC4 = new MVC4();
            //mVC4.
        ...
        {
            var u = SessionHelper.GetObjectFromJson<UserProfileSessionData>(HttpContext.Session, "UserProfileSessionData");
            var Quyen = u.ListRoleSession.Where(x => x.ActionDetailsName == "/DMChucDanh/Index/ImportDanhMuc").Count() > 0;
            if (Quyen)
            {
                ...
                if (file != null && file.Length > 0)
                {
                    ...
                    reader.Close();
                    result.Tables[0].TableName = "DMChucDanh";
                    ...
                    collection.InsertOne(emp);
                    #endregion
                    db.ImportDMChucDanh(result.Tables[0], insert);
