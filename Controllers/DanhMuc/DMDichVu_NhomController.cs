...
{
    public class DMDichVu_NhomController : BCAController
    {
        #region Khởi tạo
        private IMemoryCache cache;
        //public readonly ISession session;
        public DMDichVu_NhomController(IMemoryCache cache)
        {
            this.cache = cache;
        ...
        [HttpGet]
        public JsonResult LoadData(string maNhomDV, string tenNhomDV, string maMay, string ngaySD, string nguoiSD, int pageIndex, int pageSize, int add)
        {
            //var response = _DichVu_NhomService.DMDichVu_NhomGetListPaging(maNhomDV, tenNhomDV, maMay, ngaySD, nguoiSD, pageIndex, pageSize, add);
            //return CreateJsonJsonResult(() =>
            //{
            //    return Json(response, JsonRequestBehavior.AllowGet);
            //});
            Pagination db = new Pagination();
            if (string.IsNullOrEmpty(maNhomDV))
                maNhomDV = "";
            ...
            if (string.IsNullOrEmpty(nguoiSD))
                nguoiSD = "";
            var response = db.DMDichVu_NhomGetListPaging(maNhomDV, tenNhomDV, maMay, ngaySD, nguoiSD, pageIndex, pageSize, add);
            return CreateJsonJsonResult(() =>
            {
        ...
        [HttpPost]
        public ActionResult Modify(DMDichVu_Nhom DichVu_Nhom)
        {
            var u = SessionHelper.GetObjectFromJson<UserProfileSessionData>(HttpContext.Session, "UserProfileSessionData");
            var Quyen = u.ListRoleSession.Where(x => x.ActionDetailsName == "/DMDichVu_Nhom/Index/Modify").Count() > 0;
            if (Quyen)
            {
                ...
                emp.TenBang = "DMDichVu_Nhom";
                emp.KieuTacDong = "Update";
                emp.NguoiSD = u.Pub_sNguoiSD;
                emp.MaMay = MaMay;
                emp.NoiDungSD = JsonConvert.SerializeObject(DichVu_Nhom);
                collection.InsertOne(emp);
                #endregion
                db.spDMDichVu_Nhom_UPDATE(DichVu_Nhom.MaNhomDV, DichVu_Nhom.TenNhomDV, MaMay, u.Pub_sNguoiSD, DichVu_Nhom.Huy);
        ...
        [HttpPost]
        public ActionResult DeleteDichVu_Nhom(string maNhomDV)
        {
            var u = SessionHelper.GetObjectFromJson<UserProfileSessionData>(HttpContext.Session, "UserProfileSessionData");
            var Quyen = u.ListRoleSession.Where(x => x.ActionDetailsName == "/DMDichVu_Nhom/Index/Delete").Count() > 0;
            if (Quyen)
            {
                HL7CoreDataDataContext db = new HL7CoreDataDataContext();
                string MaMay = this.GetLocalIPAddress();
                ...
                emp.NguoiSD = u.Pub_sNguoiSD;
                emp.MaMay = MaMay;
                emp.NoiDungSD = "Delete maNhomDV: " + maNhomDV;
                collection.InsertOne(emp);
                #endregion
                db.spDMDichVu_Nhom_DELETED(maNhomDV, MaMay, u.Pub_sNguoiSD);
        ...
        [HttpPost]
        public ActionResult CreateDichVu_Nhom(string tenNhomDV)
        {
            var u = SessionHelper.GetObjectFromJson<UserProfileSessionData>(HttpContext.Session, "UserProfileSessionData");
            var Quyen = u.ListRoleSession.Where(x => x.ActionDetailsName == "/DMDichVu_Nhom/Index/Create").Count() > 0;
            if (Quyen)
            {
                string MaMay = this.GetLocalIPAddress();
                HL7CoreDataDataContext db = new HL7CoreDataDataContext();
                ...
                emp.NguoiSD = u.Pub_sNguoiSD;
                emp.MaMay = MaMay;
                emp.NoiDungSD = "Create tenNhomDV: " + tenNhomDV;
                collection.InsertOne(emp);
                #endregion
                db.spDMDichVu_Nhom_CREATE(tenNhomDV, MaMay, u.Pub_sNguoiSD);
        ...
        //public JsonResult DMDichVu_NhomGetAll()
        //{
        //    var response = _DichVu_NhomService.DMDichVu_NhomGetAll();
        //    return CreateJsonJsonResult(() =>
        //    {
        //        return Json(response, JsonRequestBehavior.AllowGet);
        //    });
        ...
        public ActionResult ExportExcel(string obj)
        {
            ...
            HL7CoreDataDataContext db = new HL7CoreDataDataContext();
            var dataSource = db.ExportDMDichVu_Nhom(key, columns);
            string filename = "DMDichVu_Nhom " + DateTime.Now.ToString() + ".xlsx";
            //MVC4 mVC4 = new MVC4();
            //mVC4.
        ...
        {
            var u = SessionHelper.GetObjectFromJson<UserProfileSessionData>(HttpContext.Session, "UserProfileSessionData");
            var Quyen = u.ListRoleSession.Where(x => x.ActionDetailsName == "/DMDichVu_Nhom/Index/ImportDanhMuc").Count() > 0;
            if (Quyen)
            {
                ...
                if (file != null && file.Length > 0)
                {
                    ...
                    reader.Close();
                    result.Tables[0].TableName = "DMDichVu_Nhom";
                    ...
                    collection.InsertOne(emp);
                    #endregion
                    db.ImportDMDichVu_Nhom(result.Tables[0], insert);
