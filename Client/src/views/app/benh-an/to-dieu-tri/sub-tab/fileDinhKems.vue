<template>
  <div class="align-center pa-2 ml-1">
    ...
    <v-progress-linear :active="!show" :indeterminate="!show" absolute bottom
      color="deep-purple accent-4"></v-progress-linear>
    <div class="mt-1" v-if="show">
      <el-upload :file-list="fileList" actions="#" :auto-upload="false" :on-change="handleUpload"
        list-type="picture-card" :on-preview="handlePictureCardPreview" :on-remove="handleRemove"
        accept=".jpg,.png,.svg,.pdf,.doc,.docx,.xlsx">
        <i class="el-icon-plus"></i>
        <template #file="{ file }">
          <div class="pa-auto ma-auto text-center flex-grow-1 justify-center">
            <img v-if="file.name.includes('jpg') || file.name.includes('png')"
              class="el-upload-list__item-thumbnail imageFile" :src="file.url" />
            <img v-else class="el-upload-list__item-thumbnail imageFile" src="@/assets/app/file.png" />
          </div>
      ...
      <el-dialog :visible.sync="dialogVisible" title="Chi tiết file">
        ...
        <v-card class="pa-2" outlined>
          <img width="100%" height="auto" :src="imageShow.url" :alt="imageShow.name" />
        </v-card>
      </el-dialog>
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    async handleRemove(file) {
      ...
      if (res) {
        var loaiTaiLieu = 6;
        const response = await deleteFile(file.idba, file.stt, loaiTaiLieu);
