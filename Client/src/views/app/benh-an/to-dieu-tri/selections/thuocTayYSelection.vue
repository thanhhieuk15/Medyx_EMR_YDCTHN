<template>
  <Fragment>
    ...
    <el-form-item prop="maThuocCus" :style="{ 'margin-bottom': 0 }">
      <portal
        :to="
          isEditForm
            ? `${crudId}-editing-${this.currentEditingRowId}-maThuocCus`
            : `${crudId}-creating-maThuocCus`
        "
      >
        <el-input
          v-model="form.maThuocCus"
...
export default {
    ...
    "form.maThuoc"(newVal) {
      if (newVal) {
        ...
        if (item) {
          ...
          this.form.hamLuong = item.hamLuong;
          this.form.donViTinh = item.donViTinh.tenDvt;
          this.form.thuocDuongDung = item.thuocDuongDung.tenDuongDung;
        }
      }
    },
  },
  methods: {
    async getThuocTayY(params) {
      return await ApiThuocTayY.ds_thuocTayY({
        ...params,
      });
    },
    async listThuocs() {
      const { data } = await ApiThuocTayY.ds_thuocTayY({isWithRelation: true});
      this.lisThuocs = data;
    },
  },
};
