      ...
      <div class="d-flex px-2">
        ...
        <div class="ml-3">
          <div style="font-size: 20px; font-weight: bold">
            Chi tiết theo dõi truyền máu
          </div>
        </div>
      </div>
      <div>
        <div class="d-flex mr-10" v-if="actions.length">
          <el-button
      ...
      >
        <el-form-item :label="`${index + 1}.${item.label}`">
          <el-input-number
            ...
            :ref="item.value"
          >
            <template #append v-if="item.unit">{{ item.unit }}</template>
          </el-input-number>
          <el-input
            v-if="item.type == 'text'"
            outlined
...
<script>
import formMixins from "./formMixins.js";
export default {
  mixins: [formMixins],
  ...
  methods: {
    ...
    goBack() {
      window.location =
        "/HSBADS/TheoDoiTruyenMau/" + window.location.href.split("/").at(-3);
    },
    // setFocus() {
    //   let itemFocus =  this.fields.find(e=>{
    //     e.action == true
    //   })[0].value;
    //   console.log
    //   this.$refs[itemFocus].$el.focus()
    // },
  },
};
</script>
