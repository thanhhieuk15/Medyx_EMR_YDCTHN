<template>
  <el-dialog
    title="Thêm mới phiếu khám dinh dưỡng"
    :visible.sync="dialog"
    width="40%"
    height="30%"
  >
...
export default {
  ...
  methods: {
    ...
    async handleCreate(formData) {
      const { statusCode } = await ApiDd.store({
        idba: window.location.href.split("/").at(-1),
        ...formData,
        huy: false,
      });
      if (statusCode == 200) {
        this.dialog = false;
        this.$emit("reset")
        this.$message.success("Thêm mới phiếu dinh dưỡng thành công !");
      } else {
        this.$message.error("Thêm mới phiếu dinh dưỡng lỗi !");
      }
    },
  },
};
