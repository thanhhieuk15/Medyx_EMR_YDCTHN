<template>
  <div>
    ...
    <div>
      <div class="mb-6">
        <HanhChinh @get-thongtinbenhnhan="getThongTinBenhNhan" />
      </div>
      <div style="margin-top: 50px">
        <QuanLyNguoiBenh :benhAn="benhAn" @get-quanLyNguoibenh="getQuanLyNguoibenh"
          :benhAnKhoaDieuTri="benhAnKhoaDieuTri" />
      </div>
      <div style="margin-top: 50px">
        <ChanDoan :benhAn="benhAn" :benhAnKhoaDieuTri="benhAnKhoaDieuTri" @get-chanDoan="getChanDoan" />
      </div>
      <div style="margin-top: 50px">
        <TinhTrangRaVien :benhAn="benhAn" @get-tinhTrangRaVien="getTinhTrangRaVien" />
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    async getKhoaDieuTru() {
      const id = window.location.href.split("/").at(-1);
      let data = await getKhoaDieuTri({ idba: id, forSelect: true });
    ...
    getThongTinBenhNhan(data) {
      for (let key in this.thongTinBenhNhan) {
        this.thongTinBenhNhan[key] = data[key];
       
      }
      console.log(this.thongTinBenhNhan);
      this.$emit('get-ThongTinBenhAn', this.benhAn, this.benhAnkdt, this.thongTinBenhNhan)
    },
    getChanDoan(data) {
      for (let key in this.benhAn) {
        if (data && data.hasOwnProperty(key)) {
