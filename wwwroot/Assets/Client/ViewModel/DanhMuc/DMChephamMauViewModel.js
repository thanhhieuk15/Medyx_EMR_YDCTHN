...
var DMChephamMauViewModel = function () {
    //khởi tạo tham số
    ...
    //mảng lưu danh mục
    self.DMChephamMaus = ko.observableArray();
    ...
    self.columns = ko.observableArray([
        ...
        { property: "MaCPMau", header: "Mã chế phẩm máu", type: "string", state: ko.observable(""), width: '10%', visible: ko.observable(true), field: '', search: 'string' },
        { property: "TenCPMau", header: "Tên chế phẩm máu(*)", type: "string", state: ko.observable(""), width: '22%', visible: ko.observable(true), field: 'input', search: 'string' },
        { property: "TenDVT", header: "Đơn vị tính", type: "string", state: ko.observable(""), width: '10%', visible: ko.observable(true), field: 'input', search: 'string' },
        { property: "TenTat", header: "Tên tắt", type: "string", state: ko.observable(""), width: '10%', visible: ko.observable(true), field: 'input', search: 'string' },
        ...
        { property: "TenBYT", header: "Tên BYT", type: "string", state: ko.observable(""), width: '10%', visible: ko.observable(true), field: 'input', search: 'string' },
    ...
    self.stringSort = function (column) { // Pass in the column object
        self.DMChephamMaus(self.DMChephamMaus().sort(function (a, b) {
    ...
    self.displayMode = function (DMChephamMau) {
        return DMChephamMau.Edit() ? 'edit-template' : 'read-template';
    }
    //sự kiện nhấn phím khi đang sửa thông tin bản ghi
    self.updateEnterInput = function (DMChephamMau, event) {
        var keyCode = (event.which ? event.which : event.keyCode);
        if (keyCode === 13) {
            DMChephamMau.TenCPMau = $('#txtTenCPMau_Edit').val();
            DMChephamMau.TenDVT = $('#txtTenDVT_Edit').val();
            DMChephamMau.TenTat = $('#txtTenTat_Edit').val();
            DMChephamMau.MaBYT = $('#txtMaBYT_Edit').val();
            DMChephamMau.TenBYT = $('#txtTenBYT_Edit').val();
            self.done(DMChephamMau);
            return false;
        }
        return true;
    }
    //sự kiện nhấn phím trong checkbox hủy cho phép lấy lại bản ghi bị xóa
    self.updateEnterHuy = function (DMChephamMau, event) {
        var keyCode = (event.which ? event.which : event.keyCode);
        if (keyCode === 32) {
            if ($('#chkHuy_edit').is(":checked")) {
                DMChephamMau.Huy = !DMChephamMau.Huy;
    ...
    //sửa thông tin
    self.edit = function (DMChephamMau) {
        if ($('#txtQuyenXem').val()=='true')
        {
            ...
            temp = DMChephamMau;
            tempTenCPMau = DMChephamMau.TenCPMau;
            tempTenDVT = DMChephamMau.TenDVT;
            tempTenTat = DMChephamMau.TenTat;
            tempMaBYT = DMChephamMau.MaBYT;
            tempTenBYT = DMChephamMau.TenBYT;
            tempHuy = DMChephamMau.Huy;
            self.AddNew(false);
            DMChephamMau.Edit(true);
            //$('#' + self.selectedItem().MaCPMau).focus();
            $('#txtTenCPMau_Edit').focus();
    ...
    self.done = function (DMChephamMau) {
        if (DMChephamMau.TenCPMau.trim() == "") {
            DMChephamMau.Edit(false);
            ...
            DMChephamMau.Edit(false);
            if (!self.AddNew()) {
                self.updateDMChephamMau(DMChephamMau);
            }
            temp = null;
            tempTenCPMau = "";
            tempTenDVT = "";
    ...
    self.subCancel = function (DMChephamMau) {
        for (var i = 0; i < self.DMChephamMaus().length; i++) {
            if (self.DMChephamMaus()[i].MaCPMau == DMChephamMau.MaCPMau) {
                self.DMChephamMaus()[i].TenCPMau = tempTenCPMau;
                self.DMChephamMaus()[i].TenDVT = tempTenDVT;
                self.DMChephamMaus()[i].TenTat = tempTenTat;
                self.DMChephamMaus()[i].MaBYT = tempMaBYT;
                self.DMChephamMaus()[i].TenBYT = tempTenBYT;
                self.DMChephamMaus()[i].Huy = tempHuy;
            }
        }
        DMChephamMau.Edit(false);
        //$("tbody").css("overflow-y", "auto");
    ...
    self.cancel = function (DMChephamMau) {
        self.subCancel(DMChephamMau);
        temp = null;
        tempTenCPMau = "";
        tempTenDVT = "";
        tempTenTat = "";
        ...
        tempTenBYT = "";
        tempHuy = false;
        var item = self.DMChephamMaus()[0];
        if (self.AddNew()) {
            self.DMChephamMaus.remove(DMChephamMau);
            self.AddNew(false)
        }
    }
    //xóa bản ghi
    self.delete = function (DMChephamMau) {
        if ($('#txtQuyenXem').val() == 'true')
        {
            if (confirm('Bạn chắc chắn muốn xóa chế phẩm máu "' + DMChephamMau.MaCPMau + ' - ' + DMChephamMau.TenCPMau + '" ?')) {
                DMChephamMau.Huy = true;
                self.deleteChephamMau(DMChephamMau);
            }
        }
    }
    //hàm xác định bản ghi đang được chọn
    self.selectItem = function (DMChephamMau) {
        self.selectedItem(DMChephamMau);
    };
    //searchResults = dmTinhs --- selectResult = selectItem --- selectedResult = selectedItem
    //move row up and down in table
    self.selectPrevious = function () {
        //var index = self.DMChephamMaus().indexOf(self.selectedItem()) - 1;
        var index = 0;
        if (self.selectedItem() != undefined) {
            if (self.selectedItem().STT > 1 && self.selectedItem().STT % self.pageSize() == 0) {
                ...
                index = self.DMChephamMaus().indexOf(self.selectedItem()) - 1;
        }
        if (index < 0) index = 0;
        self.selectedItem(self.DMChephamMaus()[index]);
        Extention.selectHeaderRow($('#' + self.selectedItem().MaCPMau), 31);
        $('#' + self.selectedItem().MaCPMau).focus();
    };
    ...
    self.selectNext = function () {
        ...
        if (self.selectedItem() != undefined)
        { index = self.DMChephamMaus().indexOf(self.selectedItem()) + 1; }
        if (index >= self.DMChephamMaus().length) index = self.DMChephamMaus().length - 1;
        self.selectedItem(self.DMChephamMaus()[index]);
        Extention.selectHeaderRow($('#' + self.selectedItem().MaCPMau), 31);
        $('#' + self.selectedItem().MaCPMau).focus();
    };
    ...
    self.upAndDown = function (data, e) {
        ...
        if (e.keyCode == 38) {
            ...
            if (this.MaCPMau != self.selectedItem().MaCPMau) {
                var index = self.DMChephamMaus().indexOf(self.selectedItem()) + 1;
                if (index >= self.DMChephamMaus().length) $('#table').focusout();
                else $('#' + self.selectedItem().MaCPMau).focus();
            }
            return false;
        }
        else return true;
    ...
    $(document).ready(function () {
        ...
        //bắt sự kiện nhấn enter ở nút nhập tên chế phẩm máu
        $('#txtiTenCPMau').keypress(function (e) {
            var code;
            if (document.all) {
                e = window.event;
                code = e.keyCode;
        ...
        //set focus vào ô nhập mới tên chế phẩm máu khi trang được load
        if ($('#txtQuyenXem').val() == 'true') {
            $('#txtiTenCPMau').focus();
        }
        else {
            $('#txtiTenCPMau').attr('disabled','disabled');
        }
    ...
    self.loadData = function () {
        ...
        var data = {
            maCPMau: $('#txtsMaCPMau').val(),
            tenCPMau: $('#txtsTenCPMau').val(),
        ...
        $.getJSON('/DMChephamMau/LoadData', data, function (data) {
            self.tongSoBanGhi(data.totalCounts);
            self.tongSoTrang = data.totalPages;
            self.DMChephamMaus(ko.utils.arrayMap(data.items, function (DMChephamMau) {
                STT += 1;
                var objDMChephamMau = {
                    STT: STT,
                    Edit: ko.observable(false),
                    MaCPMau: DMChephamMau.maCPMau,
                    TenCPMau: DMChephamMau.tenCPMau,
                    TenDVT: DMChephamMau.tenDVT,
                    TenTat: DMChephamMau.tenTat,
                    MaBYT: DMChephamMau.maBYT,
                    TenBYT: DMChephamMau.tenBYT,
                    MaMay: (DMChephamMau.maMay == undefined) ? "" : DMChephamMau.maMay,
                    Huy: DMChephamMau.huy,
                    NguoiSD: (DMChephamMau.nguoiSD == undefined) ? "" : DMChephamMau.nguoiSD,
                    NgaySD: Extention.ConvertSQLDateTimeToStringFormat(DMChephamMau.ngaySD)
                }
                return objDMChephamMau;

                if (DMChephamMau.Huy == false) {
                    $('#chkHuy_read').checked(true);
                }
            }));
            if (self.DMChephamMaus().length != 0) {
            if (self.LoadLanDau())
                self.paging();
            if (self.selectedItem() != undefined) {
                self.selectedItem().Edit(false);
                self.selectItem(self.selectedItem());
                $('#' + self.selectedItem().MaCPMau).focus().addClass('danger');
            }
            }
            else {
    ...
    self.init();
    //thêm mới bản ghi
    self.insert = function (DMChephamMau) {
        if ($('#txtQuyenXem').val() == 'true') {
            self.checkAdd(true);
            if ($('#txtiTenCPMau').val().trim() != '') {
                $.ajax({
                    url: '/DMChephamMau/CreateChephamMau',
                    type: 'POST',
                    data: {
                        tenCPMau: $('#txtiTenCPMau').val().trim(),
                        TenDVT: $('#txtiTenDVT').val().trim(),
                    ...
                    error: function (error) {
                        alert('Thêm mới không thành công! Kiểm tra lại trường nhập tên chế phẩm máu');
                    }
                });
            }
            else {
                alert('Thêm mới không thành công! Kiểm tra lại trường nhập tên chế phẩm máu');
                Extention.errorBorder('txtiTenCPMau');
                self.cancelInsert();
                $('#txtiTenCPMau').focus();
            }
        }
    ...
    //xóa bản ghi
    self.deleteChephamMau = function (DMChephamMau) {
        $.ajax({
            url: '/DMChephamMau/DeleteChephamMau',
            type: 'POST',
            data: { maCPMau: DMChephamMau.MaCPMau },
            success: function (response) {
                if (response != null && response.success == false)
                    alert(response.message);
                else {
                    if (response != null && response.success == false)
                        ...
                        alert("Xóa chế phẩm máu thành công!");
                        self.loadData();
                    }
                }
            },
            error: function (error) {
                alert("Xóa chế phẩm máu không thành công!");
            }
        })
    }
    //Import dữ liệu
    ...
    self.submitImport = function () {
        ...
        if ($('#txtQuyenXem').val() == 'true') {
            ...
            if ($('#importFile').val().trim() != '') {
                $.ajax({
                    url: '/DMChephamMau/ImportDanhMuc',
                    type: 'POST',
    ...
    self.Export = function () {
        if (self.tongSoBanGhi() <= 1048570) {
            if (self.colExport().length > 0) {
                ...
                window.open('/DMChephamMau/ExportExcel?obj=' + JSON.stringify(obj));
                //$.ajax({
                //    url: '/DMChephamMau/ExportDMChephamMau',
                //    type: 'POST',
    ...
    self.updateDMChephamMau = function (DMChephamMau) {
        $.ajax({
            url: '/DMChephamMau/Modify',
            type: 'POST',
            data: { ChephamMau: DMChephamMau },
            success: function (response) {
                if (response != null && response.success == false)
                    alert(response.message);
                else {
...
ko.applyBindings(new DMChephamMauViewModel());