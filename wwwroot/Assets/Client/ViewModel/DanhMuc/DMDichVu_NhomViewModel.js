...
var DMDichVu_NhomViewModel = function () {
    //khởi tạo tham số
    ...
    //mảng lưu danh mục
    self.DMDichVu_Nhoms = ko.observableArray();
    ...
    self.columns = ko.observableArray([
        ...
        { property: "MaNhomDV", header: "Mã nhóm dịch vụ", type: "string", state: ko.observable(""), width: '10%', visible: ko.observable(true), field: '', search: 'string' },
        { property: "TenNhomDV", header: "Tên nhóm dịch vụ(*)", type: "string", state: ko.observable(""), width: '32%', visible: ko.observable(true), field: 'input', search: 'string' },
    ...
    self.stringSort = function (column) { // Pass in the column object
        self.DMDichVu_Nhoms(self.DMDichVu_Nhoms().sort(function (a, b) {
    ...
    self.displayMode = function (DMDichVu_Nhom) {
        return DMDichVu_Nhom.Edit() ? 'edit-template' : 'read-template';
    }
    //sự kiện nhấn phím khi đang sửa thông tin bản ghi
    self.updateEnterInput = function (DMDichVu_Nhom, event) {
        var keyCode = (event.which ? event.which : event.keyCode);
        if (keyCode === 13) {
            DMDichVu_Nhom.TenNhomDV = $('#txtTenNhomDV_Edit').val();
            self.done(DMDichVu_Nhom);
            return false;
        }
        return true;
    }
    //sự kiện nhấn phím trong checkbox hủy cho phép lấy lại bản ghi bị xóa
    self.updateEnterHuy = function (DMDichVu_Nhom, event) {
        var keyCode = (event.which ? event.which : event.keyCode);
        if (keyCode === 32) {
            if ($('#chkHuy_edit').is(":checked")) {
                DMDichVu_Nhom.Huy = !DMDichVu_Nhom.Huy;
    ...
    //sửa thông tin
    self.edit = function (DMDichVu_Nhom) {
        if ($('#txtQuyenXem').val()=='true')
        {
            ...
            temp = DMDichVu_Nhom;
            tempTenNhomDV = DMDichVu_Nhom.TenNhomDV;
            tempHuy = DMDichVu_Nhom.Huy;
            self.AddNew(false);
            DMDichVu_Nhom.Edit(true);
            //$('#' + self.selectedItem().MaNhomDV).focus();
            $('#txtTenNhomDV_Edit').focus();
    ...
    self.done = function (DMDichVu_Nhom) {
        if (DMDichVu_Nhom.TenNhomDV.trim() == "") {
            DMDichVu_Nhom.Edit(false);
            ...
            DMDichVu_Nhom.Edit(false);
            if (!self.AddNew()) {
                self.updateDMDichVu_Nhom(DMDichVu_Nhom);
            }
            temp = null;
            tempTenNhomDV = "";
            tempHuy = false;
    ...
    self.subCancel = function (DMDichVu_Nhom) {
        for (var i = 0; i < self.DMDichVu_Nhoms().length; i++) {
            if (self.DMDichVu_Nhoms()[i].MaNhomDV == DMDichVu_Nhom.MaNhomDV) {
                self.DMDichVu_Nhoms()[i].TenNhomDV = tempTenNhomDV;
                self.DMDichVu_Nhoms()[i].Huy = tempHuy;
            }
        }
        DMDichVu_Nhom.Edit(false);
        //$("tbody").css("overflow-y", "auto");
    ...
    self.cancel = function (DMDichVu_Nhom) {
        self.subCancel(DMDichVu_Nhom);
        temp = null;
        tempTenNhomDV = "";
        tempHuy = false;
        var item = self.DMDichVu_Nhoms()[0];
        if (self.AddNew()) {
            self.DMDichVu_Nhoms.remove(DMDichVu_Nhom);
            self.AddNew(false)
        }
    }
    //xóa bản ghi
    self.delete = function (DMDichVu_Nhom) {
        if ($('#txtQuyenXem').val() == 'true')
        {
            if (confirm('Bạn chắc chắn muốn xóa nhóm dịch vụ "' + DMDichVu_Nhom.MaNhomDV + ' - ' + DMDichVu_Nhom.TenNhomDV + '" ?')) {
                DMDichVu_Nhom.Huy = true;
                self.deleteDichVu_Nhom(DMDichVu_Nhom);
            }
        }
    }
    //hàm xác định bản ghi đang được chọn
    self.selectItem = function (DMDichVu_Nhom) {
        self.selectedItem(DMDichVu_Nhom);
    };
    //searchResults = dmTinhs --- selectResult = selectItem --- selectedResult = selectedItem
    //move row up and down in table
    self.selectPrevious = function () {
        //var index = self.DMDichVu_Nhoms().indexOf(self.selectedItem()) - 1;
        var index = 0;
        if (self.selectedItem() != undefined) {
            if (self.selectedItem().STT > 1 && self.selectedItem().STT % self.pageSize() == 0) {
                ...
                index = self.DMDichVu_Nhoms().indexOf(self.selectedItem()) - 1;
        }
        if (index < 0) index = 0;
        self.selectedItem(self.DMDichVu_Nhoms()[index]);
        Extention.selectHeaderRow($('#' + self.selectedItem().MaNhomDV), 31);
        $('#' + self.selectedItem().MaNhomDV).focus();
    };
    ...
    self.selectNext = function () {
        ...
        if (self.selectedItem() != undefined)
        { index = self.DMDichVu_Nhoms().indexOf(self.selectedItem()) + 1; }
        if (index >= self.DMDichVu_Nhoms().length) index = self.DMDichVu_Nhoms().length - 1;
        self.selectedItem(self.DMDichVu_Nhoms()[index]);
        Extention.selectHeaderRow($('#' + self.selectedItem().MaNhomDV), 31);
        $('#' + self.selectedItem().MaNhomDV).focus();
    };
    ...
    self.upAndDown = function (data, e) {
        ...
        if (e.keyCode == 38) {
            ...
            if (this.MaNhomDV != self.selectedItem().MaNhomDV) {
                var index = self.DMDichVu_Nhoms().indexOf(self.selectedItem()) + 1;
                if (index >= self.DMDichVu_Nhoms().length) $('#table').focusout();
                else $('#' + self.selectedItem().MaNhomDV).focus();
            }
            return false;
        }
        else return true;
    ...
    $(document).ready(function () {
        ...
        //bắt sự kiện nhấn enter ở nút nhập tên nhóm dịch vụ
        $('#txtiTenNhomDV').keypress(function (e) {
            var code;
            if (document.all) {
                e = window.event;
                code = e.keyCode;
        ...
        //set focus vào ô nhập mới tên nhóm dịch vụ khi trang được load
        if ($('#txtQuyenXem').val() == 'true') {
            $('#txtiTenNhomDV').focus();
        }
        else {
            $('#txtiTenNhomDV').attr('disabled','disabled');
        }
    ...
    self.loadData = function () {
        ...
        var data = {
            MaNhomDV: $('#txtsMaNhomDV').val(),
            TenNhomDV: $('#txtsTenNhomDV').val(),
            maMay: $('#txtsMaMay').val(),
            ngaySD: ngay,
        ...
        $.getJSON('/DMDichVu_Nhom/LoadData', data, function (data) {
            self.tongSoBanGhi(data.totalCounts);
            self.tongSoTrang = data.totalPages;
            self.DMDichVu_Nhoms(ko.utils.arrayMap(data.items, function (DMDichVu_Nhom) {
                STT += 1;
                var objDMDichVu_Nhom = {
                    STT: STT,
                    Edit: ko.observable(false),
                    MaNhomDV: DMDichVu_Nhom.maNhomDV,
                    TenNhomDV: DMDichVu_Nhom.tenNhomDV,
                    MaMay: (DMDichVu_Nhom.maMay == undefined) ? "" : DMDichVu_Nhom.maMay,
                    Huy: DMDichVu_Nhom.huy,
                    NguoiSD: (DMDichVu_Nhom.nguoiSD == undefined) ? "" : DMDichVu_Nhom.nguoiSD,
                    NgaySD: (DMDichVu_Nhom.ngaySD == undefined) ? "" :Extention.ConvertSQLDateTimeToStringFormat(DMDichVu_Nhom.ngaySD)
                }
                return objDMDichVu_Nhom;

                if (DMDichVu_Nhom.Huy == false) {
                    $('#chkHuy_read').checked(true);
                }
            }));
            if (self.DMDichVu_Nhoms().length != 0) {
            if (self.LoadLanDau())
                self.paging();
            if (self.selectedItem() != undefined) {
                self.selectedItem().Edit(false);
                self.selectItem(self.selectedItem());
                $('#' + self.selectedItem().MaNhomDV).focus().addClass('danger');
            }
            }
            else {
    ...
    self.init();
    //thêm mới bản ghi
    self.insert = function (DMDichVu_Nhom) {
        if ($('#txtQuyenXem').val() == 'true') {
            self.checkAdd(true);
            if ($('#txtiTenNhomDV').val().trim() != '') {
                $.ajax({
                    url: '/DMDichVu_Nhom/CreateDichVu_Nhom',
                    type: 'POST',
                    data: { TenNhomDV: $('#txtiTenNhomDV').val().trim() },
                    ...
                    error: function (error) {
                        alert('Thêm mới không thành công! Kiểm tra lại trường nhập tên nhóm dịch vụ');
                    }
                });
            }
            else {
                alert('Thêm mới không thành công! Kiểm tra lại trường nhập tên nhóm dịch vụ');
                Extention.errorBorder('txtiTenNhomDV');
                self.cancelInsert();
                $('#txtiTenNhomDV').focus();
            }
        }
    ...
    //xóa bản ghi
    self.deleteDichVu_Nhom = function (DMDichVu_Nhom) {
        $.ajax({
            url: '/DMDichVu_Nhom/DeleteDichVu_Nhom',
            type: 'POST',
            data: { MaNhomDV: DMDichVu_Nhom.MaNhomDV },
            success: function (response) {
                if (response != null && response.success == false)
                    alert(response.message);
                else {
                    if (response != null && response.success == false)
                        ...
                        alert("Xóa nhóm dịch vụ thành công!");
                        self.loadData();
                    }
                }
            },
            error: function (error) {
                alert("Xóa nhóm dịch vụ không thành công!");
            }
        })
    }
    //Import dữ liệu
    ...
    self.submitImport = function () {
        ...
        if ($('#txtQuyenXem').val() == 'true') {
            ...
            if ($('#importFile').val().trim() != '') {
                $.ajax({
                    url: '/DMDichVu_Nhom/ImportDanhMuc',
                    type: 'POST',
    ...
    self.Export = function () {
        if (self.tongSoBanGhi() <= 1048570) {
            if (self.colExport().length > 0) {
                ...
                window.open('/DMDichVu_Nhom/ExportExcel?obj=' + JSON.stringify(obj));
                //$.ajax({
                //    url: '/DMDichVu_Nhom/ExportDMDichVu_Nhom',
                //    type: 'POST',
    ...
    self.updateDMDichVu_Nhom = function (DMDichVu_Nhom) {
        $.ajax({
            url: '/DMDichVu_Nhom/Modify',
            type: 'POST',
            data: { DichVu_Nhom: DMDichVu_Nhom },
            success: function (response) {
                if (response != null && response.success == false)
                    alert(response.message);
                else {
...
ko.applyBindings(new DMDichVu_NhomViewModel());