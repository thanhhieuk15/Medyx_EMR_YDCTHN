...
var DMVTYTViewModel = function () {
    ...
    //mảng lưu danh mục
    self.DMVTYTs = ko.observableArray();
    ...
    self.columns = ko.observableArray([
        ...
        { property: "MaVT", header: "Mã VTYT", type: "string", state: ko.observable(""), width: '10%', visible: ko.observable(true), field: '', search: 'string' },
    ...
    self.stringSort = function (column) { // Pass in the column object
        self.DMVTYTs(self.DMVTYTs().sort(function (a, b) {
    ...
    self.displayMode = function (DMVTYT) {
        return DMVTYT.Edit() ? 'edit-template' : 'read-template';
    }
    //sự kiện nhấn phím khi đang sửa thông tin bản ghi
    self.updateEnterInput = function (DMVTYT, event) {
        var keyCode = (event.which ? event.which : event.keyCode);
        if (keyCode === 13) {
            DMVTYT.MaNhom = (tempMaNhom_Edit == "") ? DMVTYT.MaNhom : tempMaNhom_Edit;
            //DMVTYT.MaVT = (tempMaPL_Edit == "") ? DMVTYT.MaVT : tempMaPL_Edit;
            //DMVTYT.MaChungLoai = (tempMaChungLoai_Edit == "") ? DMVTYT.MaChungLoai : tempMaChungLoai_Edit;
            //DMVTYT.MaDangBaoChe = (tempMaDangBaoChe_Edit == "") ? DMVTYT.MaDangBaoChe : tempMaDangBaoChe_Edit;
            DMVTYT.MaDVT = (tempMaDVT_Edit == "") ? DMVTYT.MaDVT : tempMaDVT_Edit;
            //DMVTYT.MaNSX = (tempMaNSX_Edit == "") ? DMVTYT.MaNSX : tempMaNSX_Edit;
            //DMVTYT.MaQG = (tempMaQG_Edit == "") ? DMVTYT.MaQG : tempMaQG_Edit;
            //DMVTYT.MaDuongDung = (tempMaDuongDung_Edit == "") ? DMVTYT.MaDuongDung : tempMaDuongDung_Edit;
            //DMVTYT.TenGoc = $('#txtTenGoc_Edit').val();
            DMVTYT.TenTM = $('#txtTenTM_Edit').val();
            //DMVTYT.HamLuong = $('#txtHamLuong_Edit').val();
            DMVTYT.GhiChu = $('#txtGhiChu_Edit').val();
            DMVTYT.MaBYT = $('#txtMaBYT_Edit').val();
            DMVTYT.TenBYT = $('#txtTenBYT_Edit').val();
            //DMVTYT.SoDangKy = $('#txtSoDangKy_Edit').val();
            self.done(DMVTYT);
            return false;
        }
        return true;
    ...
    self.updateEnterHuy = function (DMVTYT, event) {
        var keyCode = (event.which ? event.which : event.keyCode);
        if (keyCode === 32) {
            if ($('#chkHuy_edit').is(":checked")) {
                DMVTYT.Huy = !DMVTYT.Huy;
    ...
    self.edit = function (DMVTYT) {
        if ($('#txtQuyenXem').val()=='true')
        {
            ...
            temp = DMVTYT;
            //tempTenGoc = DMVTYT.TenGoc;
            tempTenTM = DMVTYT.TenTM;
            //tempHamLuong = DMVTYT.HamLuong;
            tempGhiChu = DMVTYT.GhiChu;
            tempMaBYT = DMVTYT.MaBYT;
            tempTenBYT = DMVTYT.TenBYT;
            //tempSoDangKy = DMVTYT.SoDangKy;
            tempHuy = DMVTYT.Huy;
            self.AddNew(false);
            DMVTYT.Edit(true);
            GeneralCategory.LoadDMVTYT_Nhom('cboTenNhom_Edit', DMVTYT.MaNhom, DMVTYT.TenNhom);
            //GeneralCategory.LoadDMVTYT_PhanLoai('cboTenPL_Edit', DMVTYT.MaVT, DMVTYT.TenPL);
            //GeneralCategory.LoadDMVTYT_ChungLoai('cboTenChungLoai_Edit', DMVTYT.MaChungLoai, DMVTYT.TenChungLoai);
            //GeneralCategory.LoadDMVTYT_DangBaoChe('cboTenDangBaoChe_Edit', DMVTYT.MaDangBaoChe, DMVTYT.TenDangBaoChe);
            GeneralCategory.LoadDMVTYT_Donvitinh('cboTenDVT_Edit', DMVTYT.MaDVT, DMVTYT.TenDVT);
            //GeneralCategory.LoadDMVTYT_DuongDung('cboTenDuongDung_Edit', DMVTYT.MaDuongDung, DMVTYT.TenDuongDung);
            //GeneralCategory.LoadDMVTYT_NhaSX('cboTenNSX_Edit', DMVTYT.MaNSX, DMVTYT.TenNSX);
            //GeneralCategory.LoadDMQuocGia('cboTenQG_Edit', DMVTYT.MaQG, DMVTYT.TenQG);
            //$('#' + self.selectedItem().MaVT).focus();
    ...
    self.done = function (DMVTYT) {
        if (DMVTYT.TenGoc.trim() == "") {
            DMVTYT.Edit(false);
    ...
    self.subCancel = function (DMVTYT) {
        for (var i = 0; i < self.DMVTYTs().length; i++) {
            if (self.DMVTYTs()[i].MaVT == DMVTYT.MaVT) {
                self.DMVTYTs()[i].MaNhom = tempMaNhom;
                //self.DMVTYTs()[i].MaVT = tempMaPL;
                //self.DMVTYTs()[i].MaChungLoai = tempMaChungLoai;
                //self.DMVTYTs()[i].MaDangBaoChe = tempMaDangBaoChe;
                self.DMVTYTs()[i].MaDVT = tempMaDVT;
                //self.DMVTYTs()[i].MaNSX = tempMaNSX;
                //self.DMVTYTs()[i].MaQG = tempMaQG;
                //self.DMVTYTs()[i].MaDuongDung = tempMaDuongDung;
                //self.DMVTYTs()[i].TenGoc = tempTenGoc;
                self.DMVTYTs()[i].TenTM = tempTenTM;
                //self.DMVTYTs()[i].HamLuong = tempHamLuong;
                self.DMVTYTs()[i].GhiChu = tempGhiChu;
                self.DMVTYTs()[i].MaBYT = tempMaBYT;
                self.DMVTYTs()[i].TenBYT = tempTenBYT;
                //self.DMVTYTs()[i].SoDangKy = tempSoDangKy;
                self.DMVTYTs()[i].Huy = tempHuy;
            }
        }
        DMVTYT.Edit(false);
        //$("tbody").css("overflow-y", "auto");
    ...
    self.cancel = function (DMVTYT) {
        self.subCancel(DMVTYT);
        temp = null;
        tempMaNhom = "";
        tempMaNhom_Edit = "";
        //tempMaPL = "";
        ...
        //tempSoDangKy = "";
        tempHuy = false;
        var item = self.DMVTYTs()[0];
        if (self.AddNew()) {
            self.DMVTYTs.remove(DMVTYT);
            self.AddNew(false)
        }
    }
    //xóa bản ghi
    self.delete = function (DMVTYT) {
        if ($('#txtQuyenXem').val() == 'true')
        {
            if (confirm('Bạn chắc chắn muốn xóa phân loại VTYT "' + DMVTYT.MaVT + ' - ' + DMVTYT.TenGoc + '" ?')) {
                DMVTYT.Huy = true;
                self.deleteVTYT(DMVTYT);
            }
        }
    }
    //hàm xác định bản ghi đang được chọn
    self.selectItem = function (DMVTYT) {
        self.selectedItem(DMVTYT);
    };
    //searchResults = dmTinhs --- selectResult = selectItem --- selectedResult = selectedItem
    //move row up and down in table
    self.selectPrevious = function () {
        //var index = self.DMVTYTs().indexOf(self.selectedItem()) - 1;
        var index = 0;
        if (self.selectedItem() != undefined) {
            if (self.selectedItem().STT > 1 && self.selectedItem().STT % self.pageSize() == 0) {
                ...
                index = self.DMVTYTs().indexOf(self.selectedItem()) - 1;
        }
        if (index < 0) index = 0;
        self.selectedItem(self.DMVTYTs()[index]);
        Extention.selectHeaderRow($('#' + self.selectedItem().MaVT), 31);
        $('#' + self.selectedItem().MaVT).focus();
    };
    ...
    self.selectNext = function () {
        ...
        if (self.selectedItem() != undefined)
        { index = self.DMVTYTs().indexOf(self.selectedItem()) + 1; }
        if (index >= self.DMVTYTs().length) index = self.DMVTYTs().length - 1;
        self.selectedItem(self.DMVTYTs()[index]);
        Extention.selectHeaderRow($('#' + self.selectedItem().MaVT), 31);
        $('#' + self.selectedItem().MaVT).focus();
    };
    ...
    self.upAndDown = function (data, e) {
        ...
        if (e.keyCode == 38) {
            ...
            if (this.MaVT != self.selectedItem().MaVT) {
                var index = self.DMVTYTs().indexOf(self.selectedItem()) + 1;
                if (index >= self.DMVTYTs().length) $('#table').focusout();
                else $('#' + self.selectedItem().MaVT).focus();
            }
            return false;
        }
        else return true;
    ...
    $(document).ready(function () {
        ...
        //bắt sự kiện nhấn enter ở nút nhập tên phân loại VTYT
        ...
        //set focus vào ô nhập mới tên phân loại VTYT khi trang được load
        if ($('#txtQuyenXem').val() == 'true') {
            $('#txtiTenPL').focus();
        }
        else {
    ...
    self.loadData = function () {
        ...
        $.getJSON('/DMVTYT/LoadData', data, function (data) {
            self.tongSoBanGhi(data.totalCounts);
            self.tongSoTrang = data.totalPages;
            self.DMVTYTs(ko.utils.arrayMap(data.items, function (DMVTYT) {
                STT += 1;
                var objDMVTYT = {
                    STT: STT,
                    Edit: ko.observable(false),
                    MaVT: DMVTYT.MaVT,
                    MaNhom: DMVTYT.maNhom,
                    TenNhom: DMVTYT.tenNhom,
                    //MaPL: DMVTYT.MaVT,
                    //TenPL: DMVTYT.tenPL,
                    //TenChungLoai: DMVTYT.tenChungLoai,
                    //MaChungLoai: DMVTYT.maChungLoai,
                    //TenDangBaoChe: DMVTYT.tenDangBaoChe,
                    //MaDangBaoChe: DMVTYT.maDangBaoChe,
                    //TenGoc: DMVTYT.tenGoc,
                    TenTM: DMVTYT.tenTM,
                    //HamLuong: DMVTYT.hamLuong,
                    MaDVT: DMVTYT.maDVT,
                    TenDVT: DMVTYT.tenDVT,
                    //MaNSX: DMVTYT.maNSX,
                    //TenNSX: DMVTYT.tenNSX,
                    GhiChu: DMVTYT.ghiChu,
                    //MaQG: DMVTYT.maQG,
                    //TenQG: DMVTYT.tenQG,
                    //MaDuongDung: DMVTYT.maDuongDung,
                    //TenDuongDung: DMVTYT.tenDuongDung,
                    MaBYT: DMVTYT.maBYT,
                    TenBYT: DMVTYT.tenBYT,
                    //SoDangKy: DMVTYT.soDangKy,
                    MaMay: (DMVTYT.maMay == undefined) ? "" : DMVTYT.maMay,
                    Huy: DMVTYT.huy,
                    NguoiSD: (DMVTYT.nguoiSD == undefined) ? "" : DMVTYT.nguoiSD,
                    NgaySD: (DMVTYT.ngaySD == undefined) ? "" :Extention.ConvertSQLDateTimeToStringFormat(DMVTYT.ngaySD)
                }
                return objDMVTYT;
            ...
            if (self.DMVTYTs().length != 0) {
            if (self.LoadLanDau())
                self.paging();
            if (self.selectedItem() != undefined) {
                self.selectedItem().Edit(false);
                self.selectItem(self.selectedItem());
                $('#' + self.selectedItem().MaVT).focus().addClass('danger');
            }
            }
            else {
    ...
    self.init();
    //thêm mới bản ghi
    self.insert = function (DMVTYT) {
        if ($('#txtQuyenXem').val() == 'true') {
            self.checkAdd(true);
            if ($('#txtiTenPL').val().trim() != '') {
                $.ajax({
                    url: '/DMVTYT/CreateVTYT',
                    type: 'POST',
                    data: {
                    ...
                    error: function (error) {
                        alert('Thêm mới không thành công! Kiểm tra lại trường nhập tên phân loại VTYT');
                    }
                });
            }
            else {
    ...
    //xóa bản ghi
    self.deleteVTYT = function (DMVTYT) {
        $.ajax({
            url: '/DMVTYT/DeleteVTYT',
            type: 'POST',
            data: { MaVT: DMVTYT.MaVT },
            success: function (response) {
                if (response != null && response.success == false)
                    alert(response.message);
                else {
                    if (response != null && response.success == false)
                        ...
                        alert("Xóa phân loại VTYT thành công!");
                        self.loadData();
                    }
                }
            },
            error: function (error) {
                alert("Xóa phân loại VTYT không thành công!");
            }
        })
    }
    //Import dữ liệu
    ...
    self.Export = function () {
        if (self.tongSoBanGhi() <= 1048570) {
            if (self.colExport().length > 0) {
                ...
                window.open('/DMVTYT/ExportExcel?obj=' + JSON.stringify(obj));
                //$.ajax({
                //    url: '/DMVTYT/ExportDMVTYT',
                //    type: 'POST',
    ...
    self.updateDMVTYT = function (DMVTYT) {
        $.ajax({
            url: '/DMVTYT/Modify',
            type: 'POST',
            data: { VTYT: DMVTYT },
    ...
    //Load danh mục nhóm VTYT
    self.LoadDMVTYT_Nhom = function () {
        if ($('#txtQuyenXem').val() == 'true') {
            GeneralCategory.LoadDMVTYT_Nhom('cboiTenNhom', false, '');
        }
        GeneralCategory.LoadDMVTYT_Nhom('cbofNhom', false, '');
    }
    self.LoadDMVTYT_Nhom();
    ////Load danh mục phân loại VTYT
    //self.LoadDMVTYT_PhanLoai = function () {
    //    if ($('#txtQuyenXem').val() == 'true') {
    //        GeneralCategory.LoadDMVTYT_PhanLoai('cboiTenPL', false, '');
    //    }
    //    GeneralCategory.LoadDMVTYT_PhanLoai('cbofPhanLoai', false, '');
    //}
    //self.LoadDMVTYT_PhanLoai();
    ////Load danh mục chủng loại VTYT
    //self.LoadDMVTYT_ChungLoai = function () {
    //    if ($('#txtQuyenXem').val() == 'true') {
    //        GeneralCategory.LoadDMVTYT_ChungLoai('cboiTenChungLoai', false, '');
    //    }
    //    GeneralCategory.LoadDMVTYT_ChungLoai('cbofChungLoai', false, '');
    //}
    //self.LoadDMVTYT_ChungLoai();
    ////Load danh mục dạng bào chế VTYT
    //self.LoadDMVTYT_DangBaoChe = function () {
    //    if ($('#txtQuyenXem').val() == 'true') {
    //        GeneralCategory.LoadDMVTYT_DangBaoChe('cboiTenDangBaoChe', false, '');
    //    }
    //    GeneralCategory.LoadDMVTYT_DangBaoChe('cbofDangBaoChe', false, '');
    //}
    //self.LoadDMVTYT_DangBaoChe();
    //Load danh mục đơn vị tính VTYT
    self.LoadDMVTYT_Donvitinh = function () {
        if ($('#txtQuyenXem').val() == 'true') {
            GeneralCategory.LoadDMVTYT_Donvitinh('cboiTenDVT', false, '');
        }
        GeneralCategory.LoadDMVTYT_Donvitinh('cbofDonvitinh', false, '');
    }
    self.LoadDMVTYT_Donvitinh();
    ////Load danh mục đường dùng VTYT
    //self.LoadDMVTYT_DuongDung = function () {
    //    if ($('#txtQuyenXem').val() == 'true') {
    //        GeneralCategory.LoadDMVTYT_DuongDung('cboiTenDuongDung', false, '');
    //    }
    //    GeneralCategory.LoadDMVTYT_DuongDung('cbofDuongDung', false, '');
    //}
    //self.LoadDMVTYT_DuongDung();
    ////Load danh mục nhà sản xuất VTYT
    //self.LoadDMVTYT_NhaSX = function () {
    //    if ($('#txtQuyenXem').val() == 'true') {
    //        GeneralCategory.LoadDMVTYT_NhaSX('cboiTenNSX', false, '');
    //    }
    //    GeneralCategory.LoadDMVTYT_NhaSX('cbofNhaSX', false, '');
    //}
    //self.LoadDMVTYT_NhaSX();
...
ko.applyBindings(new DMVTYTViewModel());