...
var DMThuoc_ChungLoaiViewModel = function () {
    //khởi tạo tham số
    ...
    //mảng lưu danh mục
    self.DMThuoc_ChungLoais = ko.observableArray();
    ...
    self.columns = ko.observableArray([
        ...
        { property: "MaChungLoai", header: "Mã chủng loại thuốc", type: "string", state: ko.observable(""), width: '10%', visible: ko.observable(true), field: '', search: 'string' },
        { property: "TenChungLoai", header: "Tên chủng loại thuốc(*)", type: "string", state: ko.observable(""), width: '32%', visible: ko.observable(true), field: 'input', search: 'string' },
    ...
    self.stringSort = function (column) { // Pass in the column object
        self.DMThuoc_ChungLoais(self.DMThuoc_ChungLoais().sort(function (a, b) {
    ...
    self.displayMode = function (DMThuoc_ChungLoai) {
        return DMThuoc_ChungLoai.Edit() ? 'edit-template' : 'read-template';
    }
    //sự kiện nhấn phím khi đang sửa thông tin bản ghi
    self.updateEnterInput = function (DMThuoc_ChungLoai, event) {
        var keyCode = (event.which ? event.which : event.keyCode);
        if (keyCode === 13) {
            DMThuoc_ChungLoai.TenChungLoai = $('#txtTenChungLoai_Edit').val();
            self.done(DMThuoc_ChungLoai);
            return false;
        }
        return true;
    }
    //sự kiện nhấn phím trong checkbox hủy cho phép lấy lại bản ghi bị xóa
    self.updateEnterHuy = function (DMThuoc_ChungLoai, event) {
        var keyCode = (event.which ? event.which : event.keyCode);
        if (keyCode === 32) {
            if ($('#chkHuy_edit').is(":checked")) {
                DMThuoc_ChungLoai.Huy = !DMThuoc_ChungLoai.Huy;
    ...
    //sửa thông tin
    self.edit = function (DMThuoc_ChungLoai) {
        if ($('#txtQuyenXem').val()=='true')
        {
            if (temp != undefined) {
                self.cancel(temp);
            }
            temp = DMThuoc_ChungLoai;
            tempTenChungLoai = DMThuoc_ChungLoai.TenChungLoai;
            tempHuy = DMThuoc_ChungLoai.Huy;
            self.AddNew(false);
            DMThuoc_ChungLoai.Edit(true);
    ...
    self.done = function (DMThuoc_ChungLoai) {
        if (DMThuoc_ChungLoai.TenChungLoai.trim() == "") {
            DMThuoc_ChungLoai.Edit(false);
            ...
            DMThuoc_ChungLoai.Edit(false);
            if (!self.AddNew()) {
                self.updateDMThuoc_ChungLoai(DMThuoc_ChungLoai);
            }
            temp = null;
            tempTenChungLoai = "";
            tempHuy = false;
    ...
    self.subCancel = function (DMThuoc_ChungLoai) {
        for (var i = 0; i < self.DMThuoc_ChungLoais().length; i++) {
            if (self.DMThuoc_ChungLoais()[i].MaChungLoai == DMThuoc_ChungLoai.MaChungLoai) {
                self.DMThuoc_ChungLoais()[i].TenChungLoai = tempTenChungLoai;
                self.DMThuoc_ChungLoais()[i].Huy = tempHuy;
            }
        }
        DMThuoc_ChungLoai.Edit(false);
        //$("tbody").css("overflow-y", "auto");
    ...
    self.cancel = function (DMThuoc_ChungLoai) {
        self.subCancel(DMThuoc_ChungLoai);
        temp = null;
        tempTenChungLoai = "";
        tempHuy = false;
        var item = self.DMThuoc_ChungLoais()[0];
        if (self.AddNew()) {
            self.DMThuoc_ChungLoais.remove(DMThuoc_ChungLoai);
            self.AddNew(false)
        }
    }
    //xóa bản ghi
    self.delete = function (DMThuoc_ChungLoai) {
        if ($('#txtQuyenXem').val() == 'true')
        {
            if (confirm('Bạn chắc chắn muốn xóa chủng loại thuốc "' + DMThuoc_ChungLoai.MaChungLoai + ' - ' + DMThuoc_ChungLoai.TenChungLoai + '" ?')) {
                DMThuoc_ChungLoai.Huy = true;
                self.deleteThuoc_ChungLoai(DMThuoc_ChungLoai);
            }
        }
    }
    //hàm xác định bản ghi đang được chọn
    self.selectItem = function (DMThuoc_ChungLoai) {
        self.selectedItem(DMThuoc_ChungLoai);
    };
    //searchResults = dmTinhs --- selectResult = selectItem --- selectedResult = selectedItem
    //move row up and down in table
    self.selectPrevious = function () {
        //var index = self.DMThuoc_ChungLoais().indexOf(self.selectedItem()) - 1;
        var index = 0;
        if (self.selectedItem() != undefined) {
            if (self.selectedItem().STT > 1 && self.selectedItem().STT % self.pageSize() == 0) {
                ...
                index = self.DMThuoc_ChungLoais().indexOf(self.selectedItem()) - 1;
        }
        if (index < 0) index = 0;
        self.selectedItem(self.DMThuoc_ChungLoais()[index]);
    ...
    self.selectNext = function () {
        ...
        if (self.selectedItem() != undefined)
        { index = self.DMThuoc_ChungLoais().indexOf(self.selectedItem()) + 1; }
        if (index >= self.DMThuoc_ChungLoais().length) index = self.DMThuoc_ChungLoais().length - 1;
        self.selectedItem(self.DMThuoc_ChungLoais()[index]);
        Extention.selectHeaderRow($('#' + self.selectedItem().MaChungLoai), 31);
        $('#' + self.selectedItem().MaChungLoai).focus();
    };
    ...
    self.upAndDown = function (data, e) {
        ...
        if (e.keyCode == 38) {
            ...
            if (this.MaChungLoai != self.selectedItem().MaChungLoai) {
                var index = self.DMThuoc_ChungLoais().indexOf(self.selectedItem()) + 1;
                if (index >= self.DMThuoc_ChungLoais().length) $('#table').focusout();
    ...
    $(document).ready(function () {
        ...
        //bắt sự kiện nhấn enter ở nút nhập tên chủng loại thuốc
        ...
        //set focus vào ô nhập mới tên chủng loại thuốc khi trang được load
        if ($('#txtQuyenXem').val() == 'true') {
    ...
    self.loadData = function () {
        ...
        $.getJSON('/DMThuoc_ChungLoai/LoadData', data, function (data) {
            self.tongSoBanGhi(data.totalCounts);
            self.tongSoTrang = data.totalPages;
            self.DMThuoc_ChungLoais(ko.utils.arrayMap(data.items, function (DMThuoc_ChungLoai) {
                STT += 1;
                var objDMThuoc_ChungLoai = {
                    STT: STT,
                    Edit: ko.observable(false),
                    MaChungLoai: DMThuoc_ChungLoai.maChungLoai,
                    TenChungLoai: DMThuoc_ChungLoai.tenChungLoai,
                    MaMay: (DMThuoc_ChungLoai.maMay == undefined) ? "" : DMThuoc_ChungLoai.maMay,
                    Huy: DMThuoc_ChungLoai.huy,
                    NguoiSD: (DMThuoc_ChungLoai.nguoiSD == undefined) ? "" : DMThuoc_ChungLoai.nguoiSD,
                    NgaySD: Extention.ConvertSQLDateTimeToStringFormat(DMThuoc_ChungLoai.ngaySD)
                }
                return objDMThuoc_ChungLoai;

                if (DMThuoc_ChungLoai.Huy == false) {
                    $('#chkHuy_read').checked(true);
                }
            }));
            if (self.DMThuoc_ChungLoais().length != 0) {
            if (self.LoadLanDau())
                self.paging();
            if (self.selectedItem() != undefined) {
    ...
    self.init();
    //thêm mới bản ghi
    self.insert = function (DMThuoc_ChungLoai) {
        if ($('#txtQuyenXem').val() == 'true') {
            self.checkAdd(true);
            if ($('#txtiTenChungLoai').val().trim() != '') {
                $.ajax({
                    url: '/DMThuoc_ChungLoai/CreateThuoc_ChungLoai',
                    type: 'POST',
                    ...
                    error: function (error) {
                        alert('Thêm mới không thành công! Kiểm tra lại trường nhập tên chủng loại thuốc');
                    }
                });
            }
            else {
    ...
    //xóa bản ghi
    self.deleteThuoc_ChungLoai = function (DMThuoc_ChungLoai) {
        $.ajax({
            url: '/DMThuoc_ChungLoai/DeleteThuoc_ChungLoai',
            type: 'POST',
            data: { maChungLoai: DMThuoc_ChungLoai.MaChungLoai },
            success: function (response) {
                if (response != null && response.success == false)
                    alert(response.message);
                else {
                    if (response != null && response.success == false)
                        ...
                        alert("Xóa chủng loại thuốc thành công!");
                        self.loadData();
                    }
                }
            },
            error: function (error) {
                alert("Xóa chủng loại thuốc không thành công!");
            }
        })
    }
    //Import dữ liệu
    ...
    self.submitImport = function () {
        ...
        if ($('#txtQuyenXem').val() == 'true') {
            ...
            if ($('#importFile').val().trim() != '') {
                $.ajax({
                    url: '/DMThuoc_ChungLoai/ImportDanhMuc',
                    type: 'POST',
    ...
    self.Export = function () {
        if (self.tongSoBanGhi() <= 1048570) {
            if (self.colExport().length > 0) {
                ...
                window.open('/DMThuoc_ChungLoai/ExportExcel?obj=' + JSON.stringify(obj));
                //$.ajax({
                //    url: '/DMThuoc_ChungLoai/ExportDMThuoc_ChungLoai',
                //    type: 'POST',
    ...
    self.updateDMThuoc_ChungLoai = function (DMThuoc_ChungLoai) {
        $.ajax({
            url: '/DMThuoc_ChungLoai/Modify',
            type: 'POST',
            data: { Thuoc_ChungLoai: DMThuoc_ChungLoai },
            success: function (response) {
                if (response != null && response.success == false)
                    alert(response.message);
                else {
...
ko.applyBindings(new DMThuoc_ChungLoaiViewModel());