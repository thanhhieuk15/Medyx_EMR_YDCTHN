...
var DMVTYT_NhomViewModel = function () {
    //khởi tạo tham số
    ...
    //mảng lưu danh mục
    self.DMVTYT_Nhoms = ko.observableArray();
    ...
    self.columns = ko.observableArray([
        ...
        { property: "MaNhom", header: "Mã nhóm VTYT", type: "string", state: ko.observable(""), width: '10%', visible: ko.observable(true), field: '', search: 'string' },
        { property: "TenNhom", header: "Tên nhóm VTYT(*)", type: "string", state: ko.observable(""), width: '22%', visible: ko.observable(true), field: 'input', search: 'string' },
    ...
    self.stringSort = function (column) { // Pass in the column object
        self.DMVTYT_Nhoms(self.DMVTYT_Nhoms().sort(function (a, b) {
    ...
    self.displayMode = function (DMVTYT_Nhom) {
        return DMVTYT_Nhom.Edit() ? 'edit-template' : 'read-template';
    }
    //sự kiện nhấn phím khi đang sửa thông tin bản ghi
    self.updateEnterInput = function (DMVTYT_Nhom, event) {
        var keyCode = (event.which ? event.which : event.keyCode);
        if (keyCode === 13) {
            DMVTYT_Nhom.TenNhom = $('#txtTenNhom_Edit').val();
            DMVTYT_Nhom.GhiChu = $('#txtGhiChu_Edit').val();
            
            self.done(DMVTYT_Nhom);
            return false;
        }
        return true;
    }
    //sự kiện nhấn phím trong checkbox hủy cho phép lấy lại bản ghi bị xóa
    self.updateEnterHuy = function (DMVTYT_Nhom, event) {
        var keyCode = (event.which ? event.which : event.keyCode);
        if (keyCode === 32) {
            if ($('#chkHuy_edit').is(":checked")) {
                DMVTYT_Nhom.Huy = !DMVTYT_Nhom.Huy;
    ...
    //sửa thông tin
    self.edit = function (DMVTYT_Nhom) {
        if ($('#txtQuyenXem').val()=='true')
        {
            ...
            temp = DMVTYT_Nhom;
            tempTenNhom = DMVTYT_Nhom.TenNhom;
            tempGhiChu = DMVTYT_Nhom.GhiChu;
            
            tempHuy = DMVTYT_Nhom.Huy;
            self.AddNew(false);
            DMVTYT_Nhom.Edit(true);
    ...
    self.done = function (DMVTYT_Nhom) {
        if (DMVTYT_Nhom.TenNhom.trim() == "") {
            DMVTYT_Nhom.Edit(false);
            ...
            DMVTYT_Nhom.Edit(false);
            if (!self.AddNew()) {
                self.updateDMVTYT_Nhom(DMVTYT_Nhom);
            }
            temp = null;
            tempTenNhom = "";
            tempGhiChu = "";
    ...
    self.subCancel = function (DMVTYT_Nhom) {
        for (var i = 0; i < self.DMVTYT_Nhoms().length; i++) {
            if (self.DMVTYT_Nhoms()[i].MaNhom == DMVTYT_Nhom.MaNhom) {
                self.DMVTYT_Nhoms()[i].TenNhom = tempTenNhom;
                self.DMVTYT_Nhoms()[i].GhiChu = tempGhiChu;
                self.DMVTYT_Nhoms()[i].Huy = tempHuy;
            }
        }
        DMVTYT_Nhom.Edit(false);
        //$("tbody").css("overflow-y", "auto");
    ...
    self.cancel = function (DMVTYT_Nhom) {
        self.subCancel(DMVTYT_Nhom);
        temp = null;
        tempTenNhom = "";
        tempGhiChu = "";
        tempHuy = false;
        var item = self.DMVTYT_Nhoms()[0];
        if (self.AddNew()) {
            self.DMVTYT_Nhoms.remove(DMVTYT_Nhom);
            self.AddNew(false)
        }
    }
    //xóa bản ghi
    self.delete = function (DMVTYT_Nhom) {
        if ($('#txtQuyenXem').val() == 'true')
        {
            if (confirm('Bạn chắc chắn muốn xóa nhóm VTYT "' + DMVTYT_Nhom.MaNhom + ' - ' + DMVTYT_Nhom.TenNhom + '" ?')) {
                DMVTYT_Nhom.Huy = true;
                self.deleteVTYT_Nhom(DMVTYT_Nhom);
            }
        }
    }
    //hàm xác định bản ghi đang được chọn
    self.selectItem = function (DMVTYT_Nhom) {
        self.selectedItem(DMVTYT_Nhom);
    };
    //searchResults = dmTinhs --- selectResult = selectItem --- selectedResult = selectedItem
    //move row up and down in table
    self.selectPrevious = function () {
        //var index = self.DMVTYT_Nhoms().indexOf(self.selectedItem()) - 1;
        var index = 0;
        if (self.selectedItem() != undefined) {
            if (self.selectedItem().STT > 1 && self.selectedItem().STT % self.pageSize() == 0) {
                ...
                index = self.DMVTYT_Nhoms().indexOf(self.selectedItem()) - 1;
        }
        if (index < 0) index = 0;
        self.selectedItem(self.DMVTYT_Nhoms()[index]);
    ...
    self.selectNext = function () {
        ...
        if (self.selectedItem() != undefined)
        { index = self.DMVTYT_Nhoms().indexOf(self.selectedItem()) + 1; }
        if (index >= self.DMVTYT_Nhoms().length) index = self.DMVTYT_Nhoms().length - 1;
        self.selectedItem(self.DMVTYT_Nhoms()[index]);
        Extention.selectHeaderRow($('#' + self.selectedItem().MaNhom), 31);
        $('#' + self.selectedItem().MaNhom).focus();
    };
    ...
    self.upAndDown = function (data, e) {
        ...
        if (e.keyCode == 38) {
            ...
            if (this.MaNhom != self.selectedItem().MaNhom) {
                var index = self.DMVTYT_Nhoms().indexOf(self.selectedItem()) + 1;
                if (index >= self.DMVTYT_Nhoms().length) $('#table').focusout();
    ...
    $(document).ready(function () {
        ...
        //bắt sự kiện nhấn enter ở nút nhập tên nhóm VTYT
        ...
        //set focus vào ô nhập mới tên nhóm VTYT khi trang được load
        if ($('#txtQuyenXem').val() == 'true') {
            $('#txtiTenNhom').focus();
        }
        else {
    ...
    self.loadData = function () {
        ...
        $.getJSON('/DMVTYT_Nhom/LoadData', data, function (data) {
            self.tongSoBanGhi(data.totalCounts);
            self.tongSoTrang = data.totalPages;
            self.DMVTYT_Nhoms(ko.utils.arrayMap(data.items, function (DMVTYT_Nhom) {
                STT += 1;
                var objDMVTYT_Nhom = {
                    STT: STT,
                    Edit: ko.observable(false),
                    MaNhom: DMVTYT_Nhom.maNhom,
                    TenNhom: DMVTYT_Nhom.tenNhom,
                    GhiChu: DMVTYT_Nhom.ghiChu,
                    MaMay: (DMVTYT_Nhom.maMay == undefined) ? "" : DMVTYT_Nhom.maMay,
                    Huy: DMVTYT_Nhom.huy,
                    NguoiSD: (DMVTYT_Nhom.nguoiSD == undefined) ? "" : DMVTYT_Nhom.nguoiSD,
                    NgaySD: (DMVTYT_Nhom.ngaySD == undefined) ? "" : Extention.ConvertSQLDateTimeToStringFormat(DMVTYT_Nhom.ngaySD)
                }
                return objDMVTYT_Nhom;

                if (DMVTYT_Nhom.Huy == false) {
                    $('#chkHuy_read').checked(true);
                }
            }));
            if (self.DMVTYT_Nhoms().length != 0) {
            if (self.LoadLanDau())
                self.paging();
            if (self.selectedItem() != undefined) {
    ...
    self.init();
    //thêm mới bản ghi
    self.insert = function (DMVTYT_Nhom) {
        if ($('#txtQuyenXem').val() == 'true') {
            self.checkAdd(true);
            if ($('#txtiTenNhom').val().trim() != '') {
                $.ajax({
                    url: '/DMVTYT_Nhom/CreateVTYT_Nhom',
                    type: 'POST',
                    ...
                    error: function (error) {
                        alert('Thêm mới không thành công! Kiểm tra lại trường nhập tên nhóm VTYT');
                    }
                });
            }
            else {
    ...
    //xóa bản ghi
    self.deleteVTYT_Nhom = function (DMVTYT_Nhom) {
        $.ajax({
            url: '/DMVTYT_Nhom/DeleteVTYT_Nhom',
            type: 'POST',
            data: { maNhom: DMVTYT_Nhom.MaNhom },
            success: function (response) {
                if (response != null && response.success == false)
                    alert(response.message);
                else {
                    if (response != null && response.success == false)
                        ...
                        alert("Xóa nhóm VTYT thành công!");
                        self.loadData();
                    }
                }
            },
            error: function (error) {
                alert("Xóa nhóm VTYT không thành công!");
            }
        })
    }
    //Import dữ liệu
    ...
    self.Export = function () {
        if (self.tongSoBanGhi() <= 1048570) {
            if (self.colExport().length > 0) {
                ...
                window.open('/DMVTYT_Nhom/ExportExcel?obj=' + JSON.stringify(obj));
                //$.ajax({
                //    url: '/DMVTYT_Nhom/ExportDMVTYT_Nhom',
                //    type: 'POST',
    ...
    self.updateDMVTYT_Nhom = function (DMVTYT_Nhom) {
        $.ajax({
            url: '/DMVTYT_Nhom/Modify',
            type: 'POST',
            data: { VTYT_Nhom: DMVTYT_Nhom },
            success: function (response) {
                if (response != null && response.success == false)
                    alert(response.message);
                else {
...
ko.applyBindings(new DMVTYT_NhomViewModel());