...
var DMThuoc_DonvitinhViewModel = function () {
    //khởi tạo tham số
    ...
    //mảng lưu danh mục
    self.DMThuoc_Donvitinhs = ko.observableArray();
    ...
    self.columns = ko.observableArray([
        ...
        { property: "MaDVT", header: "Mã đơn vị tính thuốc", type: "string", state: ko.observable(""), width: '10%', visible: ko.observable(true), field: '', search: 'string' },
        { property: "TenDVT", header: "Tên đơn vị tính thuốc(*)", type: "string", state: ko.observable(""), width: '22%', visible: ko.observable(true), field: 'input', search: 'string' },
    ...
    self.stringSort = function (column) { // Pass in the column object
        self.DMThuoc_Donvitinhs(self.DMThuoc_Donvitinhs().sort(function (a, b) {
    ...
    self.displayMode = function (DMThuoc_Donvitinh) {
        return DMThuoc_Donvitinh.Edit() ? 'edit-template' : 'read-template';
    }
    //sự kiện nhấn phím khi đang sửa thông tin bản ghi
    self.updateEnterInput = function (DMThuoc_Donvitinh, event) {
        var keyCode = (event.which ? event.which : event.keyCode);
        if (keyCode === 13) {
            DMThuoc_Donvitinh.TenDVT = $('#txtTenDVT_Edit').val();
            DMThuoc_Donvitinh.GhiChu = $('#txtGhiChu_Edit').val();
            
            self.done(DMThuoc_Donvitinh);
            return false;
        }
        return true;
    }
    //sự kiện nhấn phím trong checkbox hủy cho phép lấy lại bản ghi bị xóa
    self.updateEnterHuy = function (DMThuoc_Donvitinh, event) {
        var keyCode = (event.which ? event.which : event.keyCode);
        if (keyCode === 32) {
            if ($('#chkHuy_edit').is(":checked")) {
                DMThuoc_Donvitinh.Huy = !DMThuoc_Donvitinh.Huy;
    ...
    //sửa thông tin
    self.edit = function (DMThuoc_Donvitinh) {
        if ($('#txtQuyenXem').val()=='true')
        {
            if (temp != undefined) {
                self.cancel(temp);
            }
            temp = DMThuoc_Donvitinh;
            tempTenDVT = DMThuoc_Donvitinh.TenDVT;
            tempGhiChu = DMThuoc_Donvitinh.GhiChu;
            
            tempHuy = DMThuoc_Donvitinh.Huy;
            self.AddNew(false);
            DMThuoc_Donvitinh.Edit(true);
    ...
    self.done = function (DMThuoc_Donvitinh) {
        if (DMThuoc_Donvitinh.TenDVT.trim() == "") {
            DMThuoc_Donvitinh.Edit(false);
            ...
            DMThuoc_Donvitinh.Edit(false);
            if (!self.AddNew()) {
                self.updateDMThuoc_Donvitinh(DMThuoc_Donvitinh);
            }
            temp = null;
            tempTenDVT = "";
            tempGhiChu = "";
    ...
    self.subCancel = function (DMThuoc_Donvitinh) {
        for (var i = 0; i < self.DMThuoc_Donvitinhs().length; i++) {
            if (self.DMThuoc_Donvitinhs()[i].MaDVT == DMThuoc_Donvitinh.MaDVT) {
                self.DMThuoc_Donvitinhs()[i].TenDVT = tempTenDVT;
                self.DMThuoc_Donvitinhs()[i].GhiChu = tempGhiChu;
                
                self.DMThuoc_Donvitinhs()[i].Huy = tempHuy;
            }
        }
        DMThuoc_Donvitinh.Edit(false);
        //$("tbody").css("overflow-y", "auto");
    ...
    self.cancel = function (DMThuoc_Donvitinh) {
        self.subCancel(DMThuoc_Donvitinh);
        temp = null;
        tempTenDVT = "";
        tempGhiChu = "";
        tempHuy = false;
        var item = self.DMThuoc_Donvitinhs()[0];
        if (self.AddNew()) {
            self.DMThuoc_Donvitinhs.remove(DMThuoc_Donvitinh);
            self.AddNew(false)
        }
    }
    //xóa bản ghi
    self.delete = function (DMThuoc_Donvitinh) {
        if ($('#txtQuyenXem').val() == 'true')
        {
            if (confirm('Bạn chắc chắn muốn xóa đơn vị tính thuốc "' + DMThuoc_Donvitinh.MaDVT + ' - ' + DMThuoc_Donvitinh.TenDVT + '" ?')) {
                DMThuoc_Donvitinh.Huy = true;
                self.deleteThuoc_Donvitinh(DMThuoc_Donvitinh);
            }
        }
    }
    //hàm xác định bản ghi đang được chọn
    self.selectItem = function (DMThuoc_Donvitinh) {
        self.selectedItem(DMThuoc_Donvitinh);
    };
    //searchResults = dmTinhs --- selectResult = selectItem --- selectedResult = selectedItem
    //move row up and down in table
    self.selectPrevious = function () {
        //var index = self.DMThuoc_Donvitinhs().indexOf(self.selectedItem()) - 1;
        var index = 0;
        if (self.selectedItem() != undefined) {
            if (self.selectedItem().STT > 1 && self.selectedItem().STT % self.pageSize() == 0) {
                ...
                index = self.DMThuoc_Donvitinhs().indexOf(self.selectedItem()) - 1;
        }
        if (index < 0) index = 0;
        self.selectedItem(self.DMThuoc_Donvitinhs()[index]);
    ...
    self.selectNext = function () {
        ...
        if (self.selectedItem() != undefined)
        { index = self.DMThuoc_Donvitinhs().indexOf(self.selectedItem()) + 1; }
        if (index >= self.DMThuoc_Donvitinhs().length) index = self.DMThuoc_Donvitinhs().length - 1;
        self.selectedItem(self.DMThuoc_Donvitinhs()[index]);
        Extention.selectHeaderRow($('#' + self.selectedItem().MaDVT), 31);
        $('#' + self.selectedItem().MaDVT).focus();
    };
    ...
    self.upAndDown = function (data, e) {
        ...
        if (e.keyCode == 38) {
            ...
            if (this.MaDVT != self.selectedItem().MaDVT) {
                var index = self.DMThuoc_Donvitinhs().indexOf(self.selectedItem()) + 1;
                if (index >= self.DMThuoc_Donvitinhs().length) $('#table').focusout();
    ...
    $(document).ready(function () {
        ...
        //bắt sự kiện nhấn enter ở nút nhập tên đơn vị tính thuốc
        ...
        //set focus vào ô nhập mới tên đơn vị tính thuốc khi trang được load
        if ($('#txtQuyenXem').val() == 'true') {
            $('#txtiTenDVT').focus();
        }
        else {
    ...
    self.loadData = function () {
        ...
        $.getJSON('/DMThuoc_Donvitinh/LoadData', data, function (data) {
            self.tongSoBanGhi(data.totalCounts);
            self.tongSoTrang = data.totalPages;
            self.DMThuoc_Donvitinhs(ko.utils.arrayMap(data.items, function (DMThuoc_Donvitinh) {
                STT += 1;
                var objDMThuoc_Donvitinh = {
                    STT: STT,
                    Edit: ko.observable(false),
                    MaDVT: DMThuoc_Donvitinh.maDVT,
                    TenDVT: DMThuoc_Donvitinh.tenDVT,
                    GhiChu: DMThuoc_Donvitinh.ghiChu,
                    MaMay: (DMThuoc_Donvitinh.maMay == undefined) ? "" : DMThuoc_Donvitinh.maMay,
                    Huy: DMThuoc_Donvitinh.huy,
                    NguoiSD: (DMThuoc_Donvitinh.nguoiSD == undefined) ? "" : DMThuoc_Donvitinh.nguoiSD,
                    NgaySD: (DMThuoc_Donvitinh.ngaySD == undefined) ? "" :Extention.ConvertSQLDateTimeToStringFormat(DMThuoc_Donvitinh.ngaySD)
                }
                return objDMThuoc_Donvitinh;

                if (DMThuoc_Donvitinh.Huy == false) {
                    $('#chkHuy_read').checked(true);
                }
            }));
            if (self.DMThuoc_Donvitinhs().length != 0) {
            if (self.LoadLanDau())
                self.paging();
            if (self.selectedItem() != undefined) {
    ...
    self.init();
    //thêm mới bản ghi
    self.insert = function (DMThuoc_Donvitinh) {
        if ($('#txtQuyenXem').val() == 'true') {
            self.checkAdd(true);
            if ($('#txtiTenDVT').val().trim() != '') {
                $.ajax({
                    url: '/DMThuoc_Donvitinh/CreateThuoc_Donvitinh',
                    type: 'POST',
                    ...
                    error: function (error) {
                        alert('Thêm mới không thành công! Kiểm tra lại trường nhập tên đơn vị tính thuốc');
                    }
                });
            }
            else {
    ...
    //xóa bản ghi
    self.deleteThuoc_Donvitinh = function (DMThuoc_Donvitinh) {
        $.ajax({
            url: '/DMThuoc_Donvitinh/DeleteThuoc_Donvitinh',
            type: 'POST',
            data: { maDVT: DMThuoc_Donvitinh.MaDVT },
            success: function (response) {
                if (response != null && response.success == false)
                    alert(response.message);
                else {
                    if (response != null && response.success == false)
                        ...
                        alert("Xóa đơn vị tính thuốc thành công!");
                        self.loadData();
                    }
                }
            },
            error: function (error) {
                alert("Xóa đơn vị tính thuốc không thành công!");
            }
        })
    }
    //Import dữ liệu
    ...
    self.submitImport = function () {
        ...
        if ($('#txtQuyenXem').val() == 'true') {
            ...
            if ($('#importFile').val().trim() != '') {
                $.ajax({
                    url: '/DMThuoc_Donvitinh/ImportDanhMuc',
                    type: 'POST',
    ...
    self.Export = function () {
        if (self.tongSoBanGhi() <= 1048570) {
            if (self.colExport().length > 0) {
                ...
                window.open('/DMThuoc_Donvitinh/ExportExcel?obj=' + JSON.stringify(obj));
                //$.ajax({
                //    url: '/DMThuoc_Donvitinh/ExportDMThuoc_Donvitinh',
                //    type: 'POST',
    ...
    self.updateDMThuoc_Donvitinh = function (DMThuoc_Donvitinh) {
        $.ajax({
            url: '/DMThuoc_Donvitinh/Modify',
            type: 'POST',
            data: { Thuoc_Donvitinh: DMThuoc_Donvitinh },
            success: function (response) {
                if (response != null && response.success == false)
                    alert(response.message);
                else {
...
ko.applyBindings(new DMThuoc_DonvitinhViewModel());