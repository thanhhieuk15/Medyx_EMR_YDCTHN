...
var DMQuocGiaViewModel = function () {
    //khởi tạo tham số
    ...
    //mảng lưu danh mục
    self.DMQuocGias = ko.observableArray();
    ...
    self.columns = ko.observableArray([
        ...
        { property: "MaQG", header: "Mã Quốc Gia", type: "string", state: ko.observable(""), width: '10%', visible: ko.observable(true), field: '', search: 'string' },
        { property: "TenQG", header: "Tên Quốc Gia(*)", type: "string", state: ko.observable(""), width: '32%', visible: ko.observable(true), field: 'input', search: 'string' },
    ...
    self.stringSort = function (column) { // Pass in the column object
        self.DMQuocGias(self.DMQuocGias().sort(function (a, b) {
    ...
    self.displayMode = function (DMQuocGia) {
        return DMQuocGia.Edit() ? 'edit-template' : 'read-template';
    }
    //sự kiện nhấn phím khi đang sửa thông tin bản ghi
    self.updateEnterInput = function (DMQuocGia, event) {
        var keyCode = (event.which ? event.which : event.keyCode);
        if (keyCode === 13) {
            DMQuocGia.TenQG = $('#txtTenQG_Edit').val();
            self.done(DMQuocGia);
            return false;
        }
        return true;
    ...
    self.updateEnterHuy = function (DMQuocGia, event) {
        var keyCode = (event.which ? event.which : event.keyCode);
        if (keyCode === 32) {
            if ($('#chkHuy_edit').is(":checked")) {
                DMQuocGia.Huy = !DMQuocGia.Huy;
    ...
    self.edit = function (DMQuocGia) {
        if ($('#txtQuyenXem').val()=='true')
        {
            ...
            temp = DMQuocGia;
            tempTenQG = DMQuocGia.TenQG;
            tempHuy = DMQuocGia.Huy;
            self.AddNew(false);
            DMQuocGia.Edit(true);
            //$('#' + self.selectedItem().MaQG).focus();
            $('#txtTenQG_Edit').focus();
    ...
    self.done = function (DMQuocGia) {
        if (DMQuocGia.TenQG.trim() == "") {
            DMQuocGia.Edit(false);
            ...
            DMQuocGia.Edit(false);
            if (!self.AddNew()) {
                self.updateDMQuocGia(DMQuocGia);
            }
            temp = null;
            tempTenQG = "";
            tempHuy = false;
    ...
    self.subCancel = function (DMQuocGia) {
        for (var i = 0; i < self.DMQuocGias().length; i++) {
            if (self.DMQuocGias()[i].MaQG == DMQuocGia.MaQG) {
                self.DMQuocGias()[i].TenQG = tempTenQG;
                self.DMQuocGias()[i].Huy = tempHuy;
            }
        }
        DMQuocGia.Edit(false);
        //$("tbody").css("overflow-y", "auto");
    ...
    self.cancel = function (DMQuocGia) {
        self.subCancel(DMQuocGia);
        temp = null;
        tempTenQG = "";
        tempHuy = false;
        var item = self.DMQuocGias()[0];
        if (self.AddNew()) {
            self.DMQuocGias.remove(DMQuocGia);
            self.AddNew(false)
        }
    }
    //xóa bản ghi
    self.delete = function (DMQuocGia) {
        if ($('#txtQuyenXem').val() == 'true')
        {
            if (confirm('Bạn chắc chắn muốn xóa quốc gia "' + DMQuocGia.MaQG + ' - ' + DMQuocGia.TenQG + '" ?')) {
                DMQuocGia.Huy = true;
                self.deleteQuocGia(DMQuocGia);
            }
        }
    }
    //hàm xác định bản ghi đang được chọn
    self.selectItem = function (DMQuocGia) {
        self.selectedItem(DMQuocGia);
    };
    //searchResults = dmTinhs --- selectResult = selectItem --- selectedResult = selectedItem
    //move row up and down in table
    self.selectPrevious = function () {
        //var index = self.DMQuocGias().indexOf(self.selectedItem()) - 1;
        var index = 0;
        if (self.selectedItem() != undefined) {
            if (self.selectedItem().STT > 1 && self.selectedItem().STT % self.pageSize() == 0) {
                ...
                index = self.DMQuocGias().indexOf(self.selectedItem()) - 1;
        }
        if (index < 0) index = 0;
        self.selectedItem(self.DMQuocGias()[index]);
        Extention.selectHeaderRow($('#' + self.selectedItem().MaQG), 31);
        $('#' + self.selectedItem().MaQG).focus();
    };
    ...
    self.selectNext = function () {
        ...
        if (self.selectedItem() != undefined)
        { index = self.DMQuocGias().indexOf(self.selectedItem()) + 1; }
        if (index >= self.DMQuocGias().length) index = self.DMQuocGias().length - 1;
        self.selectedItem(self.DMQuocGias()[index]);
        Extention.selectHeaderRow($('#' + self.selectedItem().MaQG), 31);
        $('#' + self.selectedItem().MaQG).focus();
    };
    ...
    self.upAndDown = function (data, e) {
        ...
        if (e.keyCode == 38) {
            ...
            if (this.MaQG != self.selectedItem().MaQG) {
                var index = self.DMQuocGias().indexOf(self.selectedItem()) + 1;
                if (index >= self.DMQuocGias().length) $('#table').focusout();
                else $('#' + self.selectedItem().MaQG).focus();
            }
            return false;
        }
        else return true;
    ...
    $(document).ready(function () {
        ...
        if ($('#txtQuyenXem').val() == 'true') {
            ...
            $('#txtiTenQG').attr('disabled','disabled');
        }
    ...
    self.loadData = function () {
        ...
        $.getJSON('/DMQuocGia/LoadData', data, function (data) {
            self.tongSoBanGhi(data.totalCounts);
            self.tongSoTrang = data.totalPages;
            self.DMQuocGias(ko.utils.arrayMap(data.items, function (DMQuocGia) {
                STT += 1;
                var objDMQuocGia = {
                    STT: STT,
                    Edit: ko.observable(false),
                    MaQG: DMQuocGia.maQG,
                    TenQG: DMQuocGia.tenQG,
                    MaMay: (DMQuocGia.maMay == undefined) ? "" : DMQuocGia.maMay,
                    Huy: DMQuocGia.huy,
                    NguoiSD: (DMQuocGia.nguoiSD == undefined) ? "" : DMQuocGia.nguoiSD,
                    NgaySD: Extention.ConvertSQLDateTimeToStringFormat(DMQuocGia.ngaySD)
                }
                return objDMQuocGia;

                if (DMQuocGia.Huy == false) {
                    $('#chkHuy_read').checked(true);
                }
            }));
            if (self.DMQuocGias().length != 0) {
            if (self.LoadLanDau())
                self.paging();
            if (self.selectedItem() != undefined) {
                self.selectedItem().Edit(false);
                self.selectItem(self.selectedItem());
                $('#' + self.selectedItem().MaQG).focus().addClass('danger');
            }
            }
            else {
    ...
    self.init();
    //thêm mới bản ghi
    self.insert = function (DMQuocGia) {
        if ($('#txtQuyenXem').val() == 'true') {
            self.checkAdd(true);
            if ($('#txtiTenQG').val().trim() != '') {
                $.ajax({
                    url: '/DMQuocGia/CreateQuocGia',
                    type: 'POST',
                    data: { tenQG: $('#txtiTenQG').val().trim() },
                    ...
                    error: function (error) {
                        alert('Thêm mới không thành công! Kiểm tra lại trường nhập tên Quốc gia');
                    }
                });
            }
            else {
                alert('Thêm mới không thành công! Kiểm tra lại trường nhập tên Quốc gia');
                Extention.errorBorder('txtiTenQG');
                self.cancelInsert();
                $('#txtiTenQG').focus();
            }
        }
    ...
    //xóa bản ghi
    self.deleteQuocGia = function (DMQuocGia) {
        $.ajax({
            url: '/DMQuocGia/DeleteQuocGia',
            type: 'POST',
            data: { maQG: DMQuocGia.MaQG },
            success: function (response) {
                if (response != null && response.success == false)
                    alert(response.message);
                else {
            ...
            error: function (error) {
                alert("Xóa quốc gia không thành công!");
            }
        })
    }
    //Import dữ liệu
    ...
    self.Export = function () {
        if (self.tongSoBanGhi() <= 1048570) {
            if (self.colExport().length > 0) {
                ...
                window.open('/DMQuocGia/ExportDMQuocGia?obj=' + JSON.stringify(obj));
                //$.ajax({
                //    url: '/DMQuocGia/ExportDMQuocGia',
                //    type: 'POST',
    ...
    self.updateDMQuocGia = function (DMQuocGia) {
        $.ajax({
            url: '/DMQuocGia/Modify',
            type: 'POST',
            data: { QuocGia: DMQuocGia },
            success: function (response) {
                if (response != null && response.success == false)
                    alert(response.message);
                else {
...
ko.applyBindings(new DMQuocGiaViewModel());