...
var DMActionViewModel = function () {
    ...
    //mảng lưu danh mục
    self.DMActions = ko.observableArray();
    ...
    self.columns = ko.observableArray([
        ...
        { property: "MaAction", header: "Mã Action", type: "string", state: ko.observable(""), width: '10%', visible: ko.observable(true), field: '', search: 'string' },
        { property: "TenAction", header: "Tên Action(*)", type: "string", state: ko.observable(""), width: '12%', visible: ko.observable(true), field: 'input', search: 'string' },
        ...
        { property: "ButtonName", header: "Tên Button", type: "string", state: ko.observable(""), width: '10%', visible: ko.observable(true), field: 'input', search: 'string' },
        { property: "ApplicationActionID", header: "ApplicationActionID", type: "number", state: ko.observable(""), width: '10%', visible: ko.observable(true), field: 'input', search: 'number' }
    ]);
    ...
    self.stringSort = function (column) { // Pass in the column object
        self.DMActions(self.DMActions().sort(function (a, b) {
    ...
    self.displayMode = function (DMAction) {
        return DMAction.Edit() ? 'edit-template' : 'read-template';
    }
    //sự kiện nhấn phím khi đang sửa thông tin bản ghi
    self.updateEnterInput = function (DMAction, event) {
        var keyCode = (event.which ? event.which : event.keyCode);
        if (keyCode === 13) {
            DMAction.TenAction = $('#txtTenAction_Edit').val();
            self.done(DMAction);
            return false;
        }
        return true;
    ...
    self.updateEnterHuy = function (DMAction, event) {
        var keyCode = (event.which ? event.which : event.keyCode);
        if (keyCode === 32) {
            if ($('#chkHuy_edit').is(":checked")) {
                DMAction.Huy = !DMAction.Huy;
    ...
    self.edit = function (DMAction) {
        if ($('#txtQuyenXem').val()=='true')
        {
            ...
            temp = DMAction;
            tempTenAction = DMAction.TenAction;
            tempHuy = DMAction.Huy;
            tempButtonName = DMAction.ButtonName;
            tempApplicationActionID = DMAction.ApplicationActionID;
            self.AddNew(false);
            DMAction.Edit(true);
            //$('#' + self.selectedItem().MaAction).focus();
            $('#txtTenAction_Edit').focus();
    ...
    self.done = function (DMAction) {
        if (DMAction.TenAction.trim() == "") {
            DMAction.Edit(false);
            ...
            DMAction.Edit(false);
            if (!self.AddNew()) {
                self.updateDMAction(DMAction);
            }
            temp = null;
            tempTenAction = "";
            tempButtonName = "";
            tempApplicationActionID = "";
            tempHuy = false;
        }
    ...
    self.subCancel = function (DMAction) {
        for (var i = 0; i < self.DMActions().length; i++) {
            if (self.DMActions()[i].MaAction == DMAction.MaAction) {
                self.DMActions()[i].TenAction = tempTenAction;
                self.DMActions()[i].ButtonName = tempButtonName;
                self.DMActions()[i].ApplicationActionID = tempApplicationActionID;
                self.DMActions()[i].Huy = tempHuy;
            }
        }
        DMAction.Edit(false);
        //$("tbody").css("overflow-y", "auto");
    ...
    self.cancel = function (DMAction) {
        self.subCancel(DMAction);
        temp = null;
        tempTenAction = "";
        tempButtonName = "";
        tempApplicationActionID = "";
        tempHuy = false;
        var item = self.DMActions()[0];
        if (self.AddNew()) {
            self.DMActions.remove(DMAction);
            self.AddNew(false)
        }
    }
    //xóa bản ghi
    self.delete = function (DMAction) {
        if ($('#txtQuyenXem').val() == 'true')
        {
            if (confirm('Bạn chắc chắn muốn xóa Action"' + DMAction.MaAction + ' - ' + DMAction.TenAction + '" ?')) {
                DMAction.Huy = true;
                self.deleteAction(DMAction);
            }
        }
    }
    //hàm xác định bản ghi đang được chọn
    self.selectItem = function (DMAction) {
        self.selectedItem(DMAction);
    };
    //searchResults = dmTinhs --- selectResult = selectItem --- selectedResult = selectedItem
    //move row up and down in table
    self.selectPrevious = function () {
        //var index = self.DMActions().indexOf(self.selectedItem()) - 1;
        var index = 0;
        if (self.selectedItem() != undefined) {
            if (self.selectedItem().STT > 1 && self.selectedItem().STT % self.pageSize() == 0) {
                ...
                index = self.DMActions().indexOf(self.selectedItem()) - 1;
        }
        if (index < 0) index = 0;
        self.selectedItem(self.DMActions()[index]);
        Extention.selectHeaderRow($('#' + self.selectedItem().MaAction), 31);
        $('#' + self.selectedItem().MaAction).focus();
    };
    ...
    self.selectNext = function () {
        ...
        if (self.selectedItem() != undefined)
        { index = self.DMActions().indexOf(self.selectedItem()) + 1; }
        if (index >= self.DMActions().length) index = self.DMActions().length - 1;
        self.selectedItem(self.DMActions()[index]);
        Extention.selectHeaderRow($('#' + self.selectedItem().MaAction), 31);
        $('#' + self.selectedItem().MaAction).focus();
    };
    ...
    self.upAndDown = function (data, e) {
        ...
        if (e.keyCode == 38) {
            ...
            if (this.MaAction != self.selectedItem().MaAction) {
                var index = self.DMActions().indexOf(self.selectedItem()) + 1;
                if (index >= self.DMActions().length) $('#table').focusout();
                else $('#' + self.selectedItem().MaAction).focus();
            }
            return false;
        }
        else return true;
    ...
    $(document).ready(function () {
        ...
        //bắt sự kiện nhấn enter ở nút nhập tên Action
        $('#txtiTenAction').keypress(function (e) {
            var code;
            if (document.all) {
                e = window.event;
                code = e.keyCode;
        ...
        //set focus vào ô nhập mới tên Actionkhi trang được load
        if ($('#txtQuyenXem').val() == 'true') {
            $('#txtiTenAction').focus();
        }
        else {
            $('#txtiTenAction').attr('disabled','disabled');
        }
    ...
    self.loadData = function () {
        ...
        var data = {
            maAction: $('#txtsMaAction').val(),
            tenAction: $('#txtsTenAction').val(),
            maMay: $('#txtsMaMay').val(),
            ngaySD: ngay,
            ...
            add: add,
            ButtonName: $('#txtsButtonName').val(),
            ApplicationActionID: $('#txtsApplicationActionID').val()
        };
        STT = self.pageSize() * (self.pageIndex() - 1);
        $.getJSON('/DMAction/LoadData', data, function (data) {
            self.tongSoBanGhi(data.totalCounts);
            self.tongSoTrang = data.totalPages;
            self.DMActions(ko.utils.arrayMap(data.items, function (DMAction) {
                STT += 1;
                var objDMAction = {
                    STT: STT,
                    Edit: ko.observable(false),
                    MaAction: DMAction.maAction,
                    TenAction: DMAction.tenAction,
                    MaMay: (DMAction.maMay == undefined) ? "" : DMAction.maMay,
                    Huy: DMAction.huy,
                    NguoiSD: (DMAction.nguoiSD == undefined) ? "" : DMAction.nguoiSD,
                    NgaySD: (DMAction.ngaySD == undefined) ? "" : Extention.ConvertSQLDateTimeToStringFormat(DMAction.ngaySD),
                    ButtonName: DMAction.buttonName,
                    ApplicationActionID: DMAction.applicationActionID,
                }
                return objDMAction;

                if (DMAction.Huy == false) {
                    $('#chkHuy_read').checked(true);
                }
            }));
            if (self.DMActions().length != 0) {
            if (self.LoadLanDau())
                self.paging();
            if (self.selectedItem() != undefined) {
                self.selectedItem().Edit(false);
                self.selectItem(self.selectedItem());
                $('#' + self.selectedItem().MaAction).focus().addClass('danger');
            }
            }
            else {
    ...
    self.init();
    //thêm mới bản ghi
    self.insert = function (DMAction) {
        if ($('#txtQuyenXem').val() == 'true') {
            self.checkAdd(true);
            if ($('#txtiTenAction').val().trim() != '') {
                $.ajax({
                    url: '/DMAction/CreateAction',
                    type: 'POST',
                    data: { tenAction: $('#txtiTenAction').val().trim(), ButtonName: $('#txtiButtonName').val().trim(), ApplicationActionID: $('#txtiApplicationActionID').val().trim() },
                    ...
                    error: function (error) {
                        alert('Thêm mới không thành công! Kiểm tra lại trường nhập tên Action');
                    }
                });
            }
            else {
                alert('Thêm mới không thành công! Kiểm tra lại trường nhập tên Action');
                Extention.errorBorder('txtiTenAction');
                self.cancelInsert();
                $('#txtiTenAction').focus();
            }
        }
    ...
    //xóa bản ghi
    self.deleteAction = function (DMAction) {
        $.ajax({
            url: '/DMAction/DeleteAction',
            type: 'POST',
            data: { maAction: DMAction.MaAction },
            success: function (response) {
                if (response != null && response.success == false)
                    alert(response.message);
                else {
                    if (response != null && response.success == false)
                        ...
                        alert("Xóa Actionthành công!");
                        self.loadData();
                    }
                }
            },
            error: function (error) {
                alert("Xóa Actionkhông thành công!");
            }
        })
    }
    //Import dữ liệu
    ...
    self.Export = function () {
        if (self.tongSoBanGhi() <= 1048570) {
            if (self.colExport().length > 0) {
                ...
                window.open('/DMAction/ExportExcel?obj=' + JSON.stringify(obj));
                //$.ajax({
                //    url: '/DMAction/ExportDMAction',
                //    type: 'POST',
    ...
    self.updateDMAction = function (DMAction) {
        $.ajax({
            url: '/DMAction/Modify',
            type: 'POST',
            data: { Action: DMAction },
            success: function (response) {
                if (response != null && response.success == false)
                    alert(response.message);
                else {
...
ko.applyBindings(new DMActionViewModel());