...
var TraceLogViewModel = function () {
    //khởi tạo tham số
    ...
    //mảng lưu danh mục
    self.TraceLogs = ko.observableArray();
    ...
    self.columns = ko.observableArray([
        ...
        { property: "MaBN", header: "Mã BN", type: "string", state: ko.observable(""), width: '10%', visible: ko.observable(true), field: '', search: 'string' },
        { property: "ID", header: "ID", type: "string", state: ko.observable(""), width: '10%', visible: ko.observable(false), field: '', search: 'string' },
        { property: "TenBang", header: "Tên Bảng", type: "string", state: ko.observable(""), width: '12%', visible: ko.observable(true), field: 'input', search: 'string' },
        { property: "KieuTacDong", header: "Kiểu tác động", type: "string", state: ko.observable(""), width: '12%', visible: ko.observable(true), field: 'input', search: 'string' },
        { property: "NoiDungSD", header: "Nội dung", type: "string", state: ko.observable(""), width: '32%', visible: ko.observable(true), field: 'input', search: 'string' },
    ...
    self.stringSort = function (column) { // Pass in the column object
        self.TraceLogs(self.TraceLogs().sort(function (a, b) {
    ...
    self.displayMode = function (TraceLog) {
        return TraceLog.Edit() ? 'edit-template' : 'read-template';
    }
    //sự kiện nhấn phím khi đang sửa thông tin bản ghi
    self.updateEnterInput = function (TraceLog, event) {
        var keyCode = (event.which ? event.which : event.keyCode);
        if (keyCode === 13) {
            TraceLog.TenBang = $('#txtTenBang_Edit').val();
            self.done(TraceLog);
            return false;
        }
        return true;
    ...
    self.updateEnterHuy = function (TraceLog, event) {
        var keyCode = (event.which ? event.which : event.keyCode);
        if (keyCode === 32) {
            if ($('#chkHuy_edit').is(":checked")) {
                TraceLog.Huy = !TraceLog.Huy;
    ...
    self.edit = function (TraceLog) {
        if ($('#txtQuyenXem').val()=='true')
        {
            ...
            temp = TraceLog;
            tempTenBang = TraceLog.TenBang;
            tempHuy = TraceLog.Huy;
            self.AddNew(false);
            TraceLog.Edit(true);
            $('#' + self.selectedItem().MaBN).focus();
            $('#txtTenBang_Edit').focus();
    ...
    self.done = function (TraceLog) {
        if (TraceLog.TenBang.trim() == "") {
            TraceLog.Edit(false);
    ...
    self.subCancel = function (TraceLog) {
        for (var i = 0; i < self.TraceLogs().length; i++) {
            if (self.TraceLogs()[i].MaBN == TraceLog.MaBN) {
                self.TraceLogs()[i].TenBang = tempTenBang;
                self.TraceLogs()[i].Huy = tempHuy;
            }
        }
        TraceLog.Edit(false);
        //$("tbody").css("overflow-y", "auto");
    ...
    self.cancel = function (TraceLog) {
        self.subCancel(TraceLog);
        temp = null;
        tempTenBang = "";
        tempHuy = false;
        var item = self.TraceLogs()[0];
        if (self.AddNew()) {
            self.TraceLogs.remove(TraceLog);
            self.AddNew(false)
        }
    }
    //xóa bản ghi
    self.delete = function (TraceLog) {
        if ($('#txtQuyenXem').val() == 'true')
        {
            if (confirm('Bạn chắc chắn muốn xóa Log "' + TraceLog.MaBN + ' - ' + TraceLog.TenBang + '" ?')) {
                TraceLog.Huy = true;
                self.deleteChucDanh(TraceLog);
            }
        }
    }
    //hàm xác định bản ghi đang được chọn
    self.selectItem = function (TraceLog) {
        self.selectedItem(TraceLog);
    };
    //searchResults = dmTinhs --- selectResult = selectItem --- selectedResult = selectedItem
    //move row up and down in table
    self.selectPrevious = function () {
        //var index = self.TraceLogs().indexOf(self.selectedItem()) - 1;
        var index = 0;
        if (self.selectedItem() != undefined) {
            if (self.selectedItem().STT > 1 && self.selectedItem().STT % self.pageSize() == 0) {
                ...
                index = self.TraceLogs().indexOf(self.selectedItem()) - 1;
        }
        if (index < 0) index = 0;
        self.selectedItem(self.TraceLogs()[index]);
        Extention.selectHeaderRow($('#' + self.selectedItem().MaBN), 31);
        $('#' + self.selectedItem().MaBN).focus();
    };
    ...
    self.selectNext = function () {
        ...
        if (self.selectedItem() != undefined)
        { index = self.TraceLogs().indexOf(self.selectedItem()) + 1; }
        if (index >= self.TraceLogs().length) index = self.TraceLogs().length - 1;
        self.selectedItem(self.TraceLogs()[index]);
        Extention.selectHeaderRow($('#' + self.selectedItem().MaBN), 31);
        $('#' + self.selectedItem().MaBN).focus();
    };
    ...
    self.upAndDown = function (data, e) {
        ...
        if (e.keyCode == 38) {
            ...
            if (this.MaBN != self.selectedItem().MaBN) {
                var index = self.TraceLogs().indexOf(self.selectedItem()) + 1;
                if (index >= self.TraceLogs().length) $('#table').focusout();
                else $('#' + self.selectedItem().MaBN).focus();
            }
            return false;
        }
        else return true;
    ...
    $(document).ready(function () {
        ...
        //bắt sự kiện nhấn enter ở nút nhập tên Log
        $('#txtiTenBang').keypress(function (e) {
            var code;
            if (document.all) {
                e = window.event;
                code = e.keyCode;
        ...
        //set focus vào ô nhập mới tên Log khi trang được load
        if ($('#txtQuyenXem').val() == 'true') {
            $('#txtiTenBang').focus();
        }
        else {
            $('#txtiTenBang').attr('disabled','disabled');
        }
        ...
        $('#dtTuNgay, #dtDenNgay').datetimepicker({
            // dateFormat: 'dd-mm-yy',
        ...
        $("#dtTuNgay, #dtDenNgay").mask("99/99/9999 99:99");
    });
    //load dữ liệu
    self.loadData = function () {
        var tungay = Extention.strToDate($('#dtTuNgay').val());
        if (Extention.dateIsValid(tungay) == false) {
            alert("Từ ngày không hợp lệ!");
            $('#dtTuNgay').focus();
        }
        else {
            var denngay = Extention.strToDate($('#dtDenNgay').val());
            if (Extention.dateIsValid(denngay) == false) {
                ...
                var MaBN = $('#txtsMaBN').val();
                if (MaBN == undefined || MaBN.length == 0)
                    MaBN = $('#txtMaBN').val();
                var TenBang = $('#txtsTenBang').val();
                if ((TenBang == undefined || TenBang.trim().length == 0) && $('#cbofTenBang').select2('data') != undefined)
                    TenBang = $('#cbofTenBang').select2('data').id;
                var KieuTacDong = $('#txtsKieuTacDong').val();
                if ((KieuTacDong == undefined || TenBang.trim().length == 0)&& $('#cbofKieuTacDong').select2('data') != undefined)
                    KieuTacDong = $('#cbofKieuTacDong').select2('data').id;
                ...
                $.getJSON('/TraceLog/LoadData', data, function (data) {
                    self.tongSoBanGhi(data.totalCounts);
                    self.tongSoTrang = data.totalPages;
                    self.TraceLogs(ko.utils.arrayMap(data.items, function (TraceLog) {
                        STT += 1;
                        var objTraceLog = {
                            STT: STT,
                            ID: TraceLog.id,
                            ...
                            MaBN: TraceLog.maBN,
                            TenBang: TraceLog.tenBang,
                            KieuTacDong: TraceLog.kieuTacDong,
                            NoiDungSD: TraceLog.noiDungSD,
                            MaMay: (TraceLog.maMay == undefined) ? "" : TraceLog.maMay,
                            Huy: TraceLog.huy,
                            NguoiSD: (TraceLog.nguoiSD == undefined) ? "" : TraceLog.nguoiSD,
                            NgaySD: Extention.ConvertSQLDateTimeToStringFormat(TraceLog.ngaySD)
                        }
                        return objTraceLog;

                        if (TraceLog.Huy == false) {
                            $('#chkHuy_read').checked(true);
                        }
                    }));
                    if (self.TraceLogs().length != 0) {
                        if (self.LoadLanDau())
                            self.paging();
                        if (self.selectedItem() != undefined) {
                            self.selectedItem().Edit(false);
                            self.selectItem(self.selectedItem());
                            $('#' + self.selectedItem().MaBN).focus().addClass('danger');
                        }
                    }
                    else {
    ...
    self.init = function () {
        ...
        var today = new Date();
        var time = today.getDate().toString().padStart(2, '0') + '/' + (today.getMonth() + 1).toString().padStart(2, '0') + '/' + today.getFullYear();
        $('#dtTuNgay').val(time + ' 00:00');
        $('#dtDenNgay').val(time + ' 23:59');
        self.loadData();
        //self.regestEvent();
        //set ngay hien tai cho dtTuNgay, dtDenNgay
    }
    self.init();
    //thêm mới bản ghi
    self.insert = function (TraceLog) {
        if ($('#txtQuyenXem').val() == 'true') {
            self.checkAdd(true);
            if ($('#txtiTenBang').val().trim() != '') {
                $.ajax({
                    url: '/TraceLog/CreateChucDanh',
                    type: 'POST',
                    data: { TenBang: $('#txtiTenBang').val().trim() },
                    ...
                    error: function (error) {
                        alert('Thêm mới không thành công! Kiểm tra lại trường nhập tên Log');
                    }
                });
            }
            else {
                alert('Thêm mới không thành công! Kiểm tra lại trường nhập tên Log');
                Extention.errorBorder('txtiTenBang');
                self.cancelInsert();
                $('#txtiTenBang').focus();
            }
        }
    ...
    //xóa bản ghi
    self.deleteChucDanh = function (TraceLog) {
        $.ajax({
            url: '/TraceLog/DeleteChucDanh',
            type: 'POST',
            data: { MaBN: TraceLog.MaBN },
            success: function (response) {
                if (response != null && response.success == false)
                    alert(response.message);
                else {
            ...
            error: function (error) {
                alert("Xóa Log không thành công!");
            }
        })
    }
    //Import dữ liệu
    ...
    self.Export = function () {
        if (self.tongSoBanGhi() <= 1048570) {
            if (self.colExport().length > 0) {
                ...
                window.open('/TraceLog/ExportExcel?obj=' + JSON.stringify(obj));
                //$.ajax({
                //    url: '/TraceLog/ExportTraceLog',
                //    type: 'POST',
    ...
    self.updateTraceLog = function (TraceLog) {
        $.ajax({
            url: '/TraceLog/Modify',
            type: 'POST',
            data: { chucDanh: TraceLog },
            success: function (response) {
                if (response != null && response.success == false)
                    alert(response.message);
                else {
    ...
    self.LoadTraceLogTableName = function () {
        GeneralCategory.LoadTraceLogTableName('cbofTenBang', false, '');
    }
    self.LoadTraceLogTableName();
    //Load danh mục kieu tac dong ghi log
    self.LoadTraceLogKieuTacDong = function () {
        GeneralCategory.LoadTraceLogKieuTacDong('cbofKieuTacDong', false, '');
    }
    self.LoadTraceLogKieuTacDong();
...
ko.applyBindings(new TraceLogViewModel());