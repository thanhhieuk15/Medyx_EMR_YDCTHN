...
var HSBAViewModel = function () {
    ...
    self.stringSort = function (column) { // Pass in the column object
        self.HSBAs(self.HSBAs().sort(function (a, b) {
    ...
    self.displayMode = function (HSBA) {
        return HSBA.Edit() ? 'edit-template' : 'read-template';
    }
    //sự kiện nhấn phím khi đang sửa thông tin bản ghi
    self.updateEnterInput = function (HSBA, event) {
        var keyCode = (event.which ? event.which : event.keyCode);
        if (keyCode === 13) {
            HSBA.TenRole = $('#txtTenRole_Edit').val();
            self.done(HSBA);
            return false;
        }
        return true;
    ...
    self.updateEnterHuy = function (HSBA, event) {
        var keyCode = (event.which ? event.which : event.keyCode);
        if (keyCode === 32) {
    ...
    //sửa thông tin
    self.edit = function (HSBA) {
        if ($('#txtQuyenXem').val() == 'true') {
            if (temp != undefined) {
                self.cancel(temp);
            }
            temp = HSBA;
            tempTenRole = HSBA.TenRole;
            tempHuy = HSBA.Huy;
            self.AddNew(false);
            window.open('/HSBA/Index?MaRole=' + HSBA.ApplicationRolesId);
            //HSBA.Edit(true);
    ...
    self.done = function (HSBA) {
        if (HSBA.TenRole.trim() == "") {
            HSBA.Edit(false);
    ...
    self.subCancel = function (HSBA) {
        for (var i = 0; i < self.HSBAs().length; i++) {
            if (self.HSBAs()[i].ApplicationRolesId == HSBA.ApplicationRolesId) {
                self.HSBAs()[i].TenRole = tempTenRole;
                self.HSBAs()[i].Huy = tempHuy;
            }
        }
        HSBA.Edit(false);
        //$("tbody").css("overflow-y", "auto");
    ...
    self.cancel = function (HSBA) {
        self.subCancel(HSBA);
        temp = null;
        tempTenRole = "";
        tempHuy = false;
        var item = self.HSBAs()[0];
        if (self.AddNew()) {
            self.HSBAs.remove(HSBA);
            self.AddNew(false)
        }
    }
    //xóa bản ghi
    self.delete = function (HSBA) {
        if ($('#txtQuyenXem').val() == 'true') {
            if (confirm('Bạn chắc chắn muốn xóa nhóm quyền "' + HSBA.ApplicationRolesId + ' - ' + HSBA.TenRole + '" ?')) {
                HSBA.Huy = true;
                self.deleteRole(HSBA);
            }
        }
    }
    //hàm xác định bản ghi đang được chọn
    self.selectItem = function (HSBA) {
        self.selectedItem(HSBA);
    };
    //searchResults = dmTinhs --- selectResult = selectItem --- selectedResult = selectedItem
    //move row up and down in table
    self.selectPrevious = function () {
        //var index = self.HSBAs().indexOf(self.selectedItem()) - 1;
        var index = 0;
        if (self.selectedItem() != undefined) {
            if (self.selectedItem().STT > 1 && self.selectedItem().STT % self.pageSize() == 0) {
                ...
                index = self.HSBAs().indexOf(self.selectedItem()) - 1;
        }
        if (index < 0) index = 0;
        self.selectedItem(self.HSBAs()[index]);
    ...
    self.selectNext = function () {
        //var index = self.HSBAs().indexOf(self.selectedItem()) + 1;
        var index = 0;
        if (self.selectedItem() != undefined) { index = self.HSBAs().indexOf(self.selectedItem()) + 1; }
        if (index >= self.HSBAs().length) index = self.HSBAs().length - 1;
        self.selectedItem(self.HSBAs()[index]);
        Extention.selectHeaderRow($('#' + self.selectedItem().ApplicationRolesId), 31);
        $('#' + self.selectedItem().ApplicationRolesId).focus();
    };
    ...
    self.upAndDown = function (data, e) {
        ...
        if (e.keyCode == 38) {
            ...
            if (this.ApplicationRolesId != self.selectedItem().ApplicationRolesId) {
                var index = self.HSBAs().indexOf(self.selectedItem()) + 1;
                if (index >= self.HSBAs().length) $('#table').focusout();
    ...
    $(document).ready(function () {
        ...
        $('#dtNgaySinh').datepicker({
            // dateFormat: 'dd-mm-yy',
            //dateFormat: 'dd/mm/yy HH:mm:ss'
            format: 'dd/mm/yyyy',
            language: 'vi'
        });
        $("#dtNgaySinh").mask("99/99/9999");
    });
    //load dữ liệu
    self.loadData = function () {
        ...
        $.getJSON('/HSBADS/LoadData', data1, function (data) {
            ...
            self.HSBAs(ko.utils.arrayMap(data.items, function (HSBA) {
                ...
                var objHSBA = {
                    ...
                    ApplicationRolesId: HSBA.applicationRolesId,
                    TenRole: HSBA.tenRole,
                    MaMay: (HSBA.maMay == undefined) ? "" : HSBA.maMay,
                    Huy: HSBA.huy,
                    NguoiSD: (HSBA.nguoiSD == undefined) ? "" : HSBA.nguoiSD,
                    NgaySD: Extention.ConvertSQLDateTimeToStringFormat(HSBA.ngaySD)
                }
                return objHSBA;
    ...
    //xóa bản ghi
    self.deleteRole = function (HSBA) {
        $.ajax({
            url: '/HSBADS/Delete',
            type: 'POST',
            data: { ApplicationRolesId: HSBA.ApplicationRolesId },
            success: function (response) {
                if (response != null && response.success == false)
                    alert(response.message);
                else {
    ...
    self.Export = function () {
        if (self.tongSoBanGhi() <= 1048570) {
            if (self.colExport().length > 0) {
                ...
                window.open('/HSBADS/ExportExcel?obj=' + JSON.stringify(obj));
                //$.ajax({
                //    url: '/HSBADS/ExportHSBA',
                //    type: 'POST',
...
ko.applyBindings(new HSBAViewModel());